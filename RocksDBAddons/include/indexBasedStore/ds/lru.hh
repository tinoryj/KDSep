...
public:
     ...
     */
    void insert (unsigned char *key, unsigned char* value, len_t valueSize);
     ...
     * @parm key pointer to an existing instance of key 
     */
    std::string get (unsigned char *key);
     ...
     */
    bool update (unsigned char *key, unsigned char* value, len_t valueSize);
...
private:
    ...
    bool updateKey(LruListRecord* rec, unsigned char* key);
    bool updateValue(LruListRecord* rec, unsigned char* value, len_t valueSize);
    std::string getValue(LruListRecord* rec);
};
