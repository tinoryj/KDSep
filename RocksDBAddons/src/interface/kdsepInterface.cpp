...
//            StatsRecorder::getInstance()->timeProcess(StatsType::KDS_GET_READ_BUFFER_PART2, tv0);
            ...
            if (findNewValueFlag == true) {
                STAT_PROCESS(deltaKVMergeOperatorPtr_->Merge(newValueStr, buf_deltas, value), StatsType::KDS_GET_FULL_MERGE);
...
//            }
    ...
    if (ret == false) {
        ...
        if (needMergeWithInBufferOperationsFlag == true) {
            ...
            value->clear();
            bool mergeStatus;
            STAT_PROCESS(mergeStatus = deltaKVMergeOperatorPtr_->Merge(tempValueStrT, tempVec, value), StatsType::KDS_GET_FULL_MERGE);
...
bool KDSep::GetCurrentValueThenWriteBack(const string& key)
{
    ...
    STAT_PROCESS(getNewValueStrSuccessFlag = GetInternal(key, &tempRawValueStr, true), StatsType::DELTAKV_WRITE_BACK_GET);
    bool mergeStatus;
...
bool KDSep::performInBatchedBufferDeduplication(unordered_map<str_t, vector<pair<DBOperationType, mempoolHandler_t>>, mapHashKeyForStr_t, mapEqualKeForStr_t>*& operationsMap)
{
    ...
    for (auto it = operationsMap->begin(); it != operationsMap->end(); it++) {
        ...
        if (it->second.front().first == kPutOp && it->second.size() >= 2) {
            ...
            bool mergeStatus;
            STAT_PROCESS(mergeStatus = deltaKVMergeOperatorPtr_->Merge(firstValue, operandList, &finalValue), StatsType::KDS_DEDUP_FULL_MERGE);
            if (mergeStatus == false) {
            ...
            bool mergeStatus;
            STAT_PROCESS(mergeStatus = deltaKVMergeOperatorPtr_->PartialMerge(operandList, finalOperandList), StatsType::KDS_DEDUP_PARTIAL_MERGE);
            if (mergeStatus == false) {
