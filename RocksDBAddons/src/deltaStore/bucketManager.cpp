...
BucketManager::BucketManager(KDSepOptions* options, string workingDirStr, messageQueue<BucketHandler*>* notifyGCMQ, messageQueue<writeBackObject*>* writeBackOperationsQueue)
{
	...
	options->deltaStore_split_garbage_collection_start_single_file_minimum_occupancy_
	* options->deltaStore_single_file_maximum_size;
        ...
        uint64_t target_prefix, uint64_t prefix_len)
{
    ...
    STAT_PROCESS(readStatus1 = bucket1->io_ptr->readFile(readWriteBuffer1, bucket1->total_object_bytes), StatsType::DELTAKV_GC_READ);
    ...
    STAT_PROCESS(readStatus2 = bucket2->io_ptr->readFile(readWriteBuffer2, bucket2->total_object_bytes), StatsType::DELTAKV_GC_READ);
...
//            }
            ...
            STAT_PROCESS(readFileStatus = bucket->io_ptr->readFile(readWriteBuffer, bucket->total_object_bytes), StatsType::DELTAKV_GC_READ);
            ...
            if (readFileStatus.success_ == false || readFileStatus.logicalSize_ != bucket->total_object_bytes) {
                ...
                bucket->gc_status = kNoGC;
                bucket->ownership = 0;
                StatsRecorder::staticProcess(StatsType::DELTAKV_HASHSTORE_WORKER_GC, tv);
                continue;
            }
            // process GC contents
            unordered_map<str_t, pair<vector<str_t>,
...
//                    }
		...
		sizeof(KDRecordHeader);
            ...
            if (remainObjectNumberPair.first == 0 && gcResultMap.size() == 0) {
                ...
                StatsRecorder::staticProcess(StatsType::DELTAKV_HASHSTORE_WORKER_GC_BEFORE_REWRITE, tv);
