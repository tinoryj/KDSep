...
ValueLocation ValueManager::putValue(char* keyStr, key_len_t keySize, char* valueStr, len_t valueSize, const ValueLocation& oldValueLoc, bool sync, int hotness)
{
    ...
    if (!Segment::canFit(_centralizedReservedPool[poolIndex].pool, RECORD_SIZE) && !inPlaceUpdate) {
        ...
        if (ConfigManager::getInstance().usePipelinedBuffer()) {
            ...
            STAT_PROCESS(flushCentralizedReservedPool(&groupId, /* isUpdate = */ true), StatsType::POOL_FLUSH);
        }
    ...
    if (!Segment::canFit(pool, 1) || (ConfigManager::getInstance().getUpdateKVBufferSize() <= 0 && sync)) {
        ...
        if (ConfigManager::getInstance().usePipelinedBuffer()) {
            ...
            STAT_PROCESS(flushCentralizedReservedPool(/* reportGroupId* = */ 0, /* isUpdate = */ true), StatsType::POOL_FLUSH);
        }
    } else {
        //        _GCLock.unlock();
    }
...
void* ValueManager::flushCentralizedReservedPoolBgWorker(void* arg)
{
    ...
    while (instance->_started) {
        ...
        while (!instance->_centralizedReservedPoolIndex.queue.empty()) {
            ...
            STAT_PROCESS(instance->flushCentralizedReservedPool(/* *reportGroupId = */ 0, /* isUpdate = */ true, poolIndex), stats);
...
bool ValueManager::forceSync()
{
    ...
    if (ConfigManager::getInstance().enabledVLogMode() || _isSlave) {
        STAT_PROCESS(flushCentralizedReservedPoolVLog(), POOL_FLUSH);
    } else {
        STAT_PROCESS(flushCentralizedReservedPool(), POOL_FLUSH);
    }
    return true;
}
