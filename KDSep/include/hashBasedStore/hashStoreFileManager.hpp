...
public:
    HashStoreFileManager(KDSepOptions* options, std::string workingDirStr, messageQueue<hashStoreFileMetaDataHandler*>* notifyGCMQ, messageQueue<writeBackObject*>* writeBackOperationsQueue);
...
private:
    ...
    bool createFileHandlerForGC(hashStoreFileMetaDataHandler*& fileHandlerPtr,
        uint64_t targetPrefixLen, uint64_t previousFileID1, uint64_t previousFileID2, hashStoreFileHeader& newFileHeader);
