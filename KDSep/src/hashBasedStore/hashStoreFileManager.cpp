...
HashStoreFileManager::HashStoreFileManager(KDSepOptions* options, string workingDirStr, messageQueue<hashStoreFileMetaDataHandler*>* notifyGCMQ, messageQueue<writeBackObject*>* writeBackOperationsQueue)
{
    ...
    singleFileSplitGCTriggerSize_ = options->deltaStore_gc_split_threshold_ * options->deltaStore_bucket_size_;
    ...
    bool& need_flush)
{
    ...
    for (auto i = 0; i < objects.size(); i++) {
        // reserve more space
        write_buf_sz += sizeof(hashStoreRecordHeader) + objects[i].keySize_ + objects[i].valueSize_;
    }
...
bool HashStoreFileManager::cleanCommitLog()
{
    bool ret;
    struct timeval tv;
    gettimeofday(&tv, 0);
                ...
                if (openCurrentFileStatus == false) {
                    ...
                    if (file_header.file_create_reason_ == kInternalGCFile) {
                        ...
                        if (hashStoreFileIDToPrefixMap.find(file_header.previous_file_id_first_) == hashStoreFileIDToPrefixMap.end() && isGCFlushedDoneFlag == false) {
                            ...
                            // update metadata
                            hashStoreFileMetaDataHandler* tmp_hdl;
                            file_trie_.insertWithFixedBitNumber(prefix_u64,
                                file_header.prefix_bit,
                                currentRecoveryFileHandler, tmp_hdl);
                            ...
                            if (prefixBitNumberUsedInPreviousFile == file_header.prefix_bit) {
                                ...
                                if (isGCFlushedDoneFlag == true) {
                                    // gc success, keep current file, and delete old one
                                    uint64_t prefix1 = hashStoreFileIDToPrefixMap.at(file_header.previous_file_id_first_).first;
    ...
    for (auto splitFileIt : mapForBatchedkInternalGCFiles) {
        ...
        if ((exactFileNumber == 1 && gcFlagStatus[0]) == true || (exactFileNumber == 2 && gcFlagStatus[0] && gcFlagStatus[1]) == true) {
            ...
            for (int i = 0; i < exactFileNumber; i++) {
                ...
                hashStoreFileMetaDataHandler* tmp_hdl;
                file_trie_.insertWithFixedBitNumber(prefix_u64,
                    prefixBitNumber, currentRecoveryFileHandler, tmp_hdl);
                // update recovery data list
...
bool HashStoreFileManager::RetriveHashStoreFileMetaDataList()
{
    ...
    if (pointer_fs.is_open()) {
        getline(pointer_fs, currentPointerStr);
        string closeFlagStr;
    ...
    manifest_fs.open(workingDir_ + "/hashStoreFileManifestFile." + currentPointerStr, ios::in);
    string currentLineStr;
    if (manifest_fs.is_open() == false) {
        return false;
    }

    getline(manifest_fs, currentLineStr);
    targetNewFileID_ = stoull(currentLineStr) + 1; // update next file ID from metadata
    while (getline(manifest_fs, currentLineStr)) {
        ...
        hashStoreFileMetaDataHandler* tmp_hdl;
        file_trie_.insertWithFixedBitNumber(prefix_u64,
            currentFileUsedPrefixLength, file_hdl, tmp_hdl);
    }
    debug_error("insert finished %lu\n",
        maxBucketNumber_ - file_trie_.getRemainFileNumber());
...
bool HashStoreFileManager::UpdateHashStoreFileMetaDataList()
{
    ...
    manifest_fs.open(workingDir_ + "/hashStoreFileManifestFile." + to_string(currentPointerInt), ios::out);
    ...
    if (validObjectVec.size() != 0) {
        for (auto it : validObjectVec) {
            if (it.second->file_op_ptr->isFileOpen() == true) {
                ...
                manifest_fs << it.second->prefix_bit << endl;
...
bool HashStoreFileManager::prepareForUpdatingMetadata(
...
bool HashStoreFileManager::CreateHashStoreFileMetaDataListIfNotExist()
{
    ...
    uint64_t currentPointerInt = 0;
    if (pointer_fs.is_open()) {
        pointer_fs << currentPointerInt << endl;
        pointer_fs.flush();
        pointer_fs.close();
        ofstream manifest_fs;
        manifest_fs.open(workingDir_ + "/hashStoreFileManifestFile." + to_string(currentPointerInt), ios::out);
...
bool HashStoreFileManager::getFileHandlerWithKey(const char* keyBuffer,
    ...
    hashStoreFileMetaDataHandler*& file_hdl, bool getForAnchorWriting)
{
    ...
    gettimeofday(&tv, 0);
    getFileHandlerStatus = getHashStoreFileHandlerByPrefix(prefix_u64, file_hdl);
    if (op_type == kMultiPut) {
        StatsRecorder::getInstance()->timeProcess(
            StatsType::DSTORE_MULTIPUT_GET_HANDLER, tv);
    }

    if (getFileHandlerStatus == false && (op_type == kGet || op_type == kMultiGet)) {
        file_hdl = nullptr;
        return true;
    }
    ...
    if (getFileHandlerStatus == false) {
        ...
        STAT_PROCESS(createNewFileHandlerStatus = createAndGetNewHashStoreFileHandlerByPrefixForUser(prefix_u64,
                         file_hdl),
    ...
    while (true) {
        ...
        if (getFileHandlerStatus == false
            ...
            bool createNewFileHandlerStatus;
            STAT_PROCESS(createNewFileHandlerStatus = createAndGetNewHashStoreFileHandlerByPrefixForUser(
                             prefix_u64, file_hdl),
                StatsType::KDSep_HASHSTORE_CREATE_NEW_BUCKET);
            ...
            if (file_hdl->file_ownership == 1 && ((op_type == kMultiPut && file_hdl->markedByMultiPut == true) || (op_type == kMultiGet && file_hdl->markedByMultiGet == true))) {
                StatsRecorder::getInstance()->timeProcess(
            ...
            if (file_hdl->file_ownership != 0) {
                ...
                while (file_hdl->file_ownership == -1 || (file_hdl->file_ownership == 1 && (!(op_type == kMultiPut && file_hdl->markedByMultiPut) && !(op_type == kMultiGet && file_hdl->markedByMultiGet)))) {
                    gettimeofday(&tv2, 0);
...
bool HashStoreFileManager::generateHashBasedPrefix(const char* rawStr,
...
bool HashStoreFileManager::createAndGetNewHashStoreFileHandlerByPrefixForUser(const uint64_t& prefixU64, hashStoreFileMetaDataHandler*& file_hdl)
{
    hashStoreFileMetaDataHandler* tmp_file_hdl = new hashStoreFileMetaDataHandler;
    hashStoreFileMetaDataHandler* prev_file_hdl = nullptr;
    // move pointer for return
    uint64_t finalInsertLevel = file_trie_.insertWithFixedBitNumber(
        prefixU64, initialTrieBitNumber_, tmp_file_hdl, prev_file_hdl);
    if (finalInsertLevel == 0) {
        if (prev_file_hdl == nullptr) {
        ...
        delete tmp_file_hdl;
        file_hdl = prev_file_hdl;
        return true;
    }

    // initialize the file handler
    tmp_file_hdl->file_op_ptr = new FileOperation(fileOperationMethod_, maxBucketSize_,
...
bool HashStoreFileManager::createFileHandlerForGC(
    hashStoreFileMetaDataHandler*& ret_file_hdl, uint64_t targetPrefixLen, uint64_t previousFileID1, uint64_t previousFileID2,
    hashStoreFileHeader& newFileHeader)
{
    hashStoreFileMetaDataHandler* file_hdl = new hashStoreFileMetaDataHandler;
    file_hdl->file_op_ptr = new FileOperation(fileOperationMethod_, maxBucketSize_,
        singleFileFlushSize_);
    ...
    bool createAndOpenNewFileStatus = file_hdl->file_op_ptr->createThenOpenFile(targetFilePathStr);
...
HashStoreFileManager::deconstructAndGetValidContentsFromFile(
    char* read_buf, uint64_t buf_size, unordered_map<str_t, pair<vector<str_t>, vector<hashStoreRecordHeader>>, mapHashKeyForStr_t, mapEqualKeForStr_t>& resultMap)
{
...
uint64_t HashStoreFileManager::partialMergeGcResultMap(
    unordered_map<str_t, pair<vector<str_t>, vector<hashStoreRecordHeader>>, mapHashKeyForStr_t,
        ...
        mapHashKeyForStr_t, mapEqualKeForStr_t>& shouldDelete)
{
    ...
    for (auto& keyIt : gcResultMap) {
        ...
        if (values.size() >= 2) {
            ...
            newRecordHeader.sequence_number_ = headers[headers.size() - 1].sequence_number_;
    ...
    uint64_t targetFileSizeWoIndexBlock, bool fileContainsReWriteKeysFlag)
{
    ...
    if (enable_index_block_) {
        ...
        file_hdl->unsorted_part_offset = sizeof(file_header);
    }
    ...
    if (file_hdl->file_op_ptr->isFileOpen() == true) {
        ...
        if (enable_crash_consistency_) {
            STAT_PROCESS(onDiskWriteSizePair = file_hdl->file_op_ptr->writeAndFlushFile(write_buf,
                             write_i),
                StatsType::KDSep_GC_WRITE);
        } else {
            STAT_PROCESS(onDiskWriteSizePair = file_hdl->file_op_ptr->writeFile(write_buf,
                             write_i),
                StatsType::KDSep_GC_WRITE);
        }
...
bool HashStoreFileManager::singleFileSplit(hashStoreFileMetaDataHandler*
    ...
    unordered_map<str_t, pair<vector<str_t>, vector<hashStoreRecordHeader>>, mapHashKeyForStr_t,
        mapEqualKeForStr_t>& gcResultMap,
    uint64_t prefix_len, bool fileContainsReWriteKeysFlag)
{
    struct timeval tv;
    gettimeofday(&tv, 0);
    ...
    unordered_map<uint64_t, pair<unordered_map<str_t, uint64_t, mapHashKeyForStr_t, mapEqualKeForStr_t>, uint64_t>>
    ...
    for (auto keyIt : gcResultMap) {
        ...
        auto& key = keyIt.first;
        if (tempPrefixToKeysVecAndTotalSizeMap.find(prefix) != tempPrefixToKeysVecAndTotalSizeMap.end()) {
            ...
            for (auto i = 0; i < values.size(); i++) {
                ...
                currentKeyTargetAllocateSpace += key.size_ + values[i].size_ + header_sz;
            }
    ...
    for (auto prefixIt : tempPrefixToKeysVecAndTotalSizeMap) {
        ...
        uint64_t targetFileSize = prefixIt.second.second + sizeof(hashStoreFileHeader) + sizeof(hashStoreRecordHeader);
        ...
        STAT_PROCESS(onDiskWriteSizePair = new_file_hdl->file_op_ptr->writeFile(write_buf,
                         write_i),
            StatsType::KDSep_GC_WRITE);
    ...
    if (needUpdateMetaDataHandlers.size() == 1) {
        uint64_t insertAtLevel = file_trie_.insert(needUpdateMetaDataHandlers[0].first,
        ...
        pair<uint64_t, uint64_t> insertPrefixTreeStatus = file_trie_.insertPairOfNodes(needUpdateMetaDataHandlers[0].first,
            needUpdateMetaDataHandlers[0].second,
    ...
    uint64_t target_prefix, uint64_t prefix_len)
{
    ...
    std::scoped_lock<std::shared_mutex> w_lock3(file_hdl->fileOperationMutex_);
    ...
    STAT_PROCESS(readStatus1 = file_hdl1->file_op_ptr->readFile(readWriteBuffer1, file_hdl1->total_object_bytes), StatsType::KDSep_GC_READ);
    ...
    pair<uint64_t, uint64_t> remainObjectNumberPair1 = deconstructAndGetValidContentsFromFile(readWriteBuffer1,
        file_hdl1->total_object_bytes, gcResultMap1);
    if (remainObjectNumberPair1.first == 0 && remainObjectNumberPair1.second == 0) {
        debug_error("Read error: file id %lu own %d\n", file_hdl1->file_id,
            file_hdl1->file_ownership);
        exit(1);
    }
    ...
    STAT_PROCESS(readStatus2 = file_hdl2->file_op_ptr->readFile(readWriteBuffer2, file_hdl2->total_object_bytes), StatsType::KDSep_GC_READ);
    ...
    if (enable_index_block_) {
        for (auto& sorted_it : file_hdl->index_block->indices) {
            ...
            auto keyIt = gcResultMap1.find(str_t(const_cast<char*>(sorted_it.first.data()),
                sorted_it.first.size()));
            if (keyIt == gcResultMap1.end()) {
                keyIt = gcResultMap2.find(str_t(const_cast<char*>(sorted_it.first.data()),
                    sorted_it.first.size()));
        ...
        for (auto& keyIt : gcResultMap1) {
            ...
            if (keyIt.second.first.size() > 0) {
                file_hdl->filter->Insert(key.data_, key.size_);
            }
        }
    ...
    if (enable_index_block_) {
        ...
        file_hdl->unsorted_part_offset = sizeof(hashStoreFileHeader);
    }
    ...
    if (mergeNodeStatus == false) {
        ...
        file_hdl1->file_ownership = 0;
        file_hdl2->file_ownership = 0;
        if (file_hdl->index_block) {
            delete file_hdl->index_block;
        }
        delete file_hdl->file_op_ptr;
        delete file_hdl->sorted_filter;
        delete file_hdl->filter;
        delete file_hdl;
        ...
        bool updateFileHandlerToNewLeafNodeStatus = file_trie_.updateDataObjectForTargetLeafNode(target_prefix,
            prefix_len, newLeafNodeBitNumber, file_hdl);
    ...
    uint64_t& target_prefix, uint64_t& prefix_len)
{
    ...
    for (auto& nodeIt : validNodes) {
        ...
        if (file_hdl->total_object_bytes <= singleFileMergeGCUpperBoundSize_ && file_trie_.isMergableLength(prefixLenExtract(nodeIt.first)) && file_hdl->file_ownership != -1) {
    ...
    hashStoreFileMetaDataHandler *sel_hdl1, *sel_hdl2;
    sel_hdl1 = sel_hdl2 = nullptr;

    uint64_t sel_prefix_len = 0;
    uint64_t sel_target = 0;

    vector<hashStoreFileMetaDataHandler*> selected;
    ...
    for (auto mapIt : targetFileForMergeMap) {
        ...
        if (file_trie_.get(prefix2, tempHandler, prefix_len2) == true) {
            ...
            uint64_t total_bytes = tempHandler->total_object_bytes + another_hdl->total_object_bytes;
            if (total_bytes < sel_threshold) {
                if (enableBatchedOperations_ == true) {
                    if (another_hdl->file_ownership != 0 || tempHandler->file_ownership != 0) {
                        continue;
                        // skip wait if batched op
                    }

                    // skip the should delete files
                    if (another_hdl->gc_status == kShouldDelete || tempHandler->gc_status == kShouldDelete) {
                        continue;
                    }
                }
                if (another_hdl->file_ownership != 0) {
                    debug_info("Stop this merge for file ID = %lu\n",
                        another_hdl->file_id);
                    continue;
                }
                another_hdl->file_ownership = -1;
                // check again to see whether the assignment is correct
                if (another_hdl->file_ownership != -1) {
                    continue;
                }
                ...
                file_hdl1 = another_hdl;
                if (tempHandler->file_ownership != 0) {
                    another_hdl->file_ownership = 0;
                    debug_info("Stop this merge for file ID = %lu\n", tempHandler->file_id);
                    continue;
                }
                ...
                if (tempHandler->file_ownership != -1) {
                    if (file_hdl1->file_ownership == -1) {
                        file_hdl1->file_ownership = 0;
                        continue;
                    }
                }
                ...
                //			file_hdl1->file_ownership = 0;
                //			file_hdl2->file_ownership = 0;
                //			selected.push_back(file_hdl1);
                //			selected.push_back(file_hdl2);
                //			prefices_needed.push_back(target_prefix);
                //			prefices_needed.push_back(prefix_len);

                if (sel_hdl1 != nullptr && sel_hdl1 != file_hdl1 && sel_hdl1 != file_hdl2 && sel_hdl2 != file_hdl1 && sel_hdl2 != file_hdl2) {
                    sel_hdl1->file_ownership = 0;
                    sel_hdl2->file_ownership = 0;
                }
    ...
    while (true) {
        ...
        if (selectFileForMergeStatus == true) {
            debug_info("Select two file for merge GC success, "
                       "file_hdl 1 id = %p, file_hdl 2 ptr = %p, "
                       "target prefix = %lx\n",
                file_hdl1, file_hdl2, merge_prefix);
            if (merge_cnt % 10 == 0) {
                debug_error("Select two file for merge id1 %lu id2 %lu "
                            "total size %lu + %lu = %lu, prefix bit %lu,"
                            " merge cnt %d\n",
                    file_hdl1->file_id,
                    file_hdl2->file_id,
                    file_hdl1->total_object_bytes,
                    file_hdl2->total_object_bytes,
                    file_hdl1->total_object_bytes + file_hdl2->total_object_bytes,
                    file_hdl1->prefix_bit,
                    merge_cnt);
            }
            merge_cnt++;
            bool performFileMergeStatus = twoAdjacentFileMerge(file_hdl1, file_hdl2,
...
void HashStoreFileManager::processSingleFileGCRequestWorker(int threadID)
{
    ...
    while (true) {
        ...
        if (notifyGCMQ_->pop(file_hdl)) {
            ...
            STAT_PROCESS(readFileStatus = file_hdl->file_op_ptr->readFile(readWriteBuffer, file_hdl->total_object_bytes), StatsType::KDSep_GC_READ);
            ...
            if (readFileStatus.success_ == false || readFileStatus.logicalSize_ != file_hdl->total_object_bytes) {
                ...
                file_hdl->gc_status = kNoGC;
                file_hdl->file_ownership = 0;
                StatsRecorder::getInstance()->timeProcess(StatsType::KDSep_HASHSTORE_WORKER_GC, tv);
                continue;
            }
            // process GC contents
            unordered_map<str_t, pair<vector<str_t>, vector<hashStoreRecordHeader>>, mapHashKeyForStr_t,
            ...
            STAT_PROCESS(remainObjectNumberPair = deconstructAndGetValidContentsFromFile(readWriteBuffer,
            ...
            if (enableLsmTreeDeltaMeta_ == false) {
                STAT_PROCESS(
                    remainObjectNumberPair.first -= partialMergeGcResultMap(gcResultMap, shouldDelete),
            ...
            for (auto keyIt : gcResultMap) {
                ...
                if (enableWriteBackDuringGCFlag_ == true) {
                    ...
                    if ((keyIt.second.first.size() > gcWriteBackDeltaNum_ && gcWriteBackDeltaNum_ != 0) || (total_kd_size > gcWriteBackDeltaSize_ && gcWriteBackDeltaSize_ != 0)) {
            ...
            uint64_t targetFileSizeWoIndexBlock = target_size + sizeof(hashStoreFileHeader) + sizeof(hashStoreRecordHeader);
            ...
            if (remainObjectNumberPair.first == 0 && gcResultMap.size() == 0) {
                ...
                StatsRecorder::getInstance()->timeProcess(StatsType::KDSep_HASHSTORE_WORKER_GC_BEFORE_REWRITE, tv);
