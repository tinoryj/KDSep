...
public interface AdvancedMutableColumnFamilyOptionsInterface<T extends AdvancedMutableColumnFamilyOptionsInterface<T>> {
     ...
     *                                            while overflowing the underlying
     ...
     *
     *         See
     *         {@link MutableColumnFamilyOptionsInterface#setMaxBytesForLevelBase(long)}
     */
     ...
     *
     *         See
     *         {@link MutableColumnFamilyOptionsInterface#maxBytesForLevelBase()}
     */
     ...
     * {@link RocksDB#setOptions(ColumnFamilyHandle, MutableColumnFamilyOptions)}.
     *
     * @return the current minimum size of value which is stored separately in a
     *         blob
     */
    long minBlobSize();
     ...
     * Default: false
     *
     * @param enableBlobGarbageCollection the new enabled/disabled state of blob
     *                                    garbage collection
     *
     * @return the reference to the current options.
     */
    ...
    /**
     * Query whether garbage collection of blobs is enabled.Blob GC is performed as
     * part of
     * compaction. Valid blobs residing in blob files older than a cutoff get
     * relocated to new files as they are encountered during compaction, which makes
     * it possible to clean up blob files once they contain nothing but
     ...
     * @return the reference to the current options.
     */
    T setEnableDeltaFiles(final boolean enableDeltaFiles);
     ...
     * {@link RocksDB#setOptions(ColumnFamilyHandle, MutableColumnFamilyOptions)}.
     *
     * @return true if delta files are enabled
     */
    boolean enableDeltaFiles();
     ...
     * @return the reference to the current options.
     */
    T setMinDeltaSize(final long minDeltaSize);
     ...
     * {@link RocksDB#setOptions(ColumnFamilyHandle, MutableColumnFamilyOptions)}.
     *
     * @param deltaFileSize the size limit for delta files
     *
     * @return the reference to the current options.
     */
    T setDeltaFileSize(final long deltaFileSize);
     ...
     * once this limit is reached.
     *
     * @return the current size limit for delta files
     */
    long deltaFileSize();
     ...
     * @return the reference to the current options.
     */
    T setDeltaCompressionType(CompressionType compressionType);

    /**
     * Get the compression algorithm in use for large values stored in delta files.
     ...
     * Default: false
     *
     * @param enableDeltaGarbageCollection the new enabled/disabled state of delta
     *                                     garbage collection
     *
     * @return the reference to the current options.
     */
    T setEnableDeltaGarbageCollection(final boolean enableDeltaGarbageCollection);

    /**
     * Query whether garbage collection of deltas is enabled.Delta GC is performed
     * as part of
     * compaction. Valid deltas residing in delta files older than a cutoff get
     * relocated to new files as they are encountered during compaction, which makes
     * it possible to clean up delta files once they contain nothing but
     ...
     * Default: false
     *
     * @return true if delta garbage collection is currently enabled.
     */
    boolean enableDeltaGarbageCollection();

    /**
     * Set cutoff in terms of delta file age for garbage collection. Deltas in the
     ...
     * Default: 0.25
     *
     * @param deltaGarbageCollectionAgeCutoff the new age cutoff
     *
     * @return the reference to the current options.
     */
    T setDeltaGarbageCollectionAgeCutoff(double deltaGarbageCollectionAgeCutoff);

    /**
     * Get cutoff in terms of delta file age for garbage collection. Deltas in the
     * oldest N delta files will be relocated when encountered during compaction,
     * where N = garbage_collection_cutoff * number_of_delta_files. Note that
     * enable_delta_garbage_collection has to be set in order for this option to
     * have
     ...
     * @return the current age cutoff for garbage collection
     */
    double deltaGarbageCollectionAgeCutoff();
     ...
     * @return the reference to the current options
     */
    T setDeltaGarbageCollectionForceThreshold(double deltaGarbageCollectionForceThreshold);
     ...
     */
    double deltaGarbageCollectionForceThreshold();
     ...
     * @return the reference to the current options.
     */
    T setDeltaCompactionReadaheadSize(final long deltaCompactionReadaheadSize);
     ...
     */
    long deltaCompactionReadaheadSize();
     ...
     * @return the reference to the current options.
     */
    T setDeltaFileStartingLevel(final int deltaFileStartingLevel);
     ...
     * @return the reference to the current options.
     */
    T setPrepopulateDeltaCache(final PrepopulateDeltaCache prepopulateDeltaCache);
     ...
     */
    PrepopulateDeltaCache prepopulateDeltaCache();
