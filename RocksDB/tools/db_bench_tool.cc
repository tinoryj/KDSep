...
DEFINE_bool(use_cache_jemalloc_no_dump_allocator, false,
            "Use JemallocNodumpAllocator for block/blob/delta cache.");

DEFINE_bool(use_cache_memkind_kmem_allocator, false,
            "Use memkind kmem allocator for block/blob/delta cache.");
...
// Integrated DeltaDB options
DEFINE_bool(enable_delta_files,
            ROCKSDB_NAMESPACE::AdvancedColumnFamilyOptions().enable_delta_files,
            "[Integrated DeltaDB] Enable writing large values to separate "
            "delta files.");

DEFINE_uint64(
    min_delta_size,
    ROCKSDB_NAMESPACE::AdvancedColumnFamilyOptions().min_delta_size,
    "[Integrated DeltaDB] The size of the smallest value to be stored "
    "separately in a delta file.");

DEFINE_uint64(delta_file_size,
              ROCKSDB_NAMESPACE::AdvancedColumnFamilyOptions().delta_file_size,
              "[Integrated DeltaDB] The size limit for delta files.");

DEFINE_string(delta_compression_type, "none",
              "[Integrated DeltaDB] The compression algorithm to use for large "
              "values stored in delta files.");

DEFINE_bool(enable_delta_garbage_collection,
            ROCKSDB_NAMESPACE::AdvancedColumnFamilyOptions()
                .enable_delta_garbage_collection,
            "[Integrated DeltaDB] Enable delta garbage collection.");

DEFINE_double(delta_garbage_collection_age_cutoff,
              ROCKSDB_NAMESPACE::AdvancedColumnFamilyOptions()
                  .delta_garbage_collection_age_cutoff,
              "[Integrated DeltaDB] The cutoff in terms of delta file age for "
              "garbage collection.");

DEFINE_double(delta_garbage_collection_force_threshold,
              ROCKSDB_NAMESPACE::AdvancedColumnFamilyOptions()
                  .delta_garbage_collection_force_threshold,
              "[Integrated DeltaDB] The threshold for the ratio of garbage in "
              "the oldest delta files for forcing garbage collection.");

DEFINE_uint64(delta_compaction_readahead_size,
              ROCKSDB_NAMESPACE::AdvancedColumnFamilyOptions()
                  .delta_compaction_readahead_size,
              "[Integrated DeltaDB] Compaction readahead for delta files.");

DEFINE_int32(
    delta_file_starting_level,
    ROCKSDB_NAMESPACE::AdvancedColumnFamilyOptions().delta_file_starting_level,
    "[Integrated DeltaDB] The starting level for delta files.");

DEFINE_bool(use_delta_cache, false, "[Integrated DeltaDB] Enable delta cache.");

DEFINE_bool(use_shared_block_and_delta_cache, true,
            "[Integrated DeltaDB] Use a shared backing cache for both block "
            "cache and delta cache. It only takes effect if use_delta_cache is "
            "enabled.");

DEFINE_uint64(
    delta_cache_size, 8 << 20,
    "[Integrated DeltaDB] Number of bytes to use as a cache of deltas. It only "
    "takes effect if the block and delta caches are different "
    "(use_shared_block_and_delta_cache = false).");

DEFINE_int32(
    delta_cache_numshardbits, 6,
    "[Integrated DeltaDB] Number of shards for the delta cache is 2 ** "
    "delta_cache_numshardbits. Negative means use default settings. "
    "It only takes effect if delta_cache_size is greater than 0, and "
    "the block and delta caches are different "
    "(use_shared_block_and_delta_cache = false).");

DEFINE_int32(
    prepopulate_delta_cache, 0,
    "[Integrated DeltaDB] Pre-populate hot/warm deltas in delta cache. 0 "
    "to disable and 1 to insert during flush.");
...
DEFINE_bool(charge_delta_cache, false,
            "Setting for "
            "CacheEntryRoleOptions::charged of "
...
class Benchmark {
 private:
  ...
  bool use_delta_db_;   // Stacked DeltaDB
 ...
 public:
  Benchmark()
      : cache_(NewCache(FLAGS_cache_size)),
        ...
        use_delta_db_(FLAGS_use_delta_db),  // DeltaDB
#else
        use_blob_db_(false),   // Stacked BlobDB
        use_delta_db_(false),  // DeltaDB
#endif  // !ROCKSDB_LITE
    ...
    if (!FLAGS_use_existing_db) {
      ...
      if (use_delta_db_) {
        // Stacked DeltaDB
        delta_db::DestroyDeltaDB(FLAGS_db, options, delta_db::DeltaDBOptions());
      }
#endif  // !ROCKSDB_LITE
