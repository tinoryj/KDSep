...
struct JobContext {
  inline bool HaveSomethingToDelete() const {
    return !(full_scan_candidate_files.empty() && sst_delete_files.empty() &&
             blob_delete_files.empty() && delta_delete_files.empty() &&
  ...
  // the list of all live delta files that cannot be deleted
  std::vector<uint64_t> delta_live;

  // the list of delta files that we need to delete
  std::vector<ObsoleteDeltaFileInfo> delta_delete_files;
