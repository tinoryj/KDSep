...
const char* GetCompactionReasonString(CompactionReason compaction_reason) {
  switch (compaction_reason) {
    ...
    case CompactionReason::kForcedBlobGC:
      return "ForcedBlobGC";
    case CompactionReason::kForcedDeltaGC:
      return "ForcedDeltaGC";
...
CompactionJob::CompactionJob(
    ...
    FSDirectory* delta_output_directory, Statistics* stats,
    ...
    DeltaFileCompletionCallback* delta_callback, int* bg_compaction_scheduled,
    ...
    : compact_(new CompactionState(compaction)),
      ...
      delta_output_directory_(delta_output_directory),
      db_mutex_(db_mutex),
...
Status CompactionJob::Run() {
  ...
  bool wrote_new_delta_files = false;
  ...
  for (const auto& state : compact_->sub_compact_states) {
    ...
    if (state.Current().HasDeltaFileAdditions()) {
      wrote_new_delta_files = true;
    }
  }
  ...
  if (status.ok()) {
    ...
    if (io_s.ok() && wrote_new_delta_files && delta_output_directory_ &&
        delta_output_directory_ != output_directory_) {
      io_s = delta_output_directory_->FsyncWithDirOptions(
          IOOptions(), dbg,
          DirFsyncOptions(DirFsyncOptions::FsyncReason::kNewFileSynced));
    }
  }
...
Status CompactionJob::Install(const MutableCFOptions& mutable_cf_options) {
  ...
  const uint64_t bytes_read_non_output_and_blob_and_delta =
      stats.bytes_read_non_output_levels + stats.bytes_read_blob +
      stats.bytes_read_delta;
  const uint64_t bytes_read_all =
      stats.bytes_read_output_level + bytes_read_non_output_and_blob_and_delta;
  const uint64_t bytes_written_all = stats.bytes_written +
                                     stats.bytes_written_blob +
                                     stats.bytes_written_delta;

  if (bytes_read_non_output_and_blob_and_delta > 0) {
    read_write_amp =
        (bytes_written_all + bytes_read_all) /
        static_cast<double>(bytes_read_non_output_and_blob_and_delta);
    write_amp = bytes_written_all /
                static_cast<double>(bytes_read_non_output_and_blob_and_delta);
  }
  if (stats.micros > 0) {
  ...
  if (!blob_files.empty()) {
    ...
    ROCKS_LOG_BUFFER(
        log_buffer_,
        "[%s] compacted to: %s, MB/sec: %.1f rd, %.1f wr, level %d, "
        "files in(%d, %d) out(%d +%d delta) "
        "MB in(%.1f, %.1f +%.1f delta) out(%.1f +%.1f delta), "
        "read-write-amplify(%.1f) write-amplify(%.1f) %s, records in: %" PRIu64
        ", records dropped: %" PRIu64 " output_compression: %s\n",
        column_family_name.c_str(), vstorage->LevelSummary(&tmp),
        bytes_read_per_sec, bytes_written_per_sec,
        ...
        stats.num_input_files_in_non_output_levels,
        stats.num_input_files_in_output_level, stats.num_output_files,
        stats.num_output_files_delta, stats.bytes_read_non_output_levels / kMB,
        stats.bytes_read_output_level / kMB, stats.bytes_read_delta / kMB,
        stats.bytes_written / kMB, stats.bytes_written_delta / kMB,
    ...
    if (stats.num_output_files_delta > 0) {
      stream << "num_delta_output_files" << stats.num_output_files_delta
             << "total_delta_output_size" << stats.bytes_written_delta;
    }
    ...
    if (compaction_stats_.has_penultimate_level_output) {
      ...
      stream << "penultimate_level_num_output_files"
             << pl_stats.num_output_files;
      ...
      stream << "penultimate_level_num_output_files_delta"
             << pl_stats.num_output_files_delta;
      stream << "penultimate_level_bytes_written_delta"
             << pl_stats.bytes_written_delta;
    }
  ...
  void CompactionJob::NotifyOnSubcompactionBegin(SubcompactionState *
                                                 sub_compact) {
#ifndef ROCKSDB_LITE
  ...
  void CompactionJob::NotifyOnSubcompactionCompleted(SubcompactionState *
                                                     sub_compact) {
#ifndef ROCKSDB_LITE
  ...
  void CompactionJob::ProcessKeyValueCompaction(SubcompactionState *
                                                sub_compact) {
    assert(sub_compact);
    ...
    if (end.has_value()) {
      end_ikey.SetInternalKey(end.value(), kMaxSequenceNumber,
                              kValueTypeForSeek);
      if (ts_sz > 0) {
    ...
    std::unique_ptr<InternalIterator> delta_counter;

    if (sub_compact->compaction->DoesInputReferenceDeltaFiles()) {
      DeltaGarbageMeter* meter =
          sub_compact->Current().CreateDeltaGarbageMeter();
      delta_counter = std::make_unique<DeltaCountingIterator>(input, meter);
      input = delta_counter.get();
    }
    ...
    // TODO: BlobDB to support output_to_penultimate_level compaction, which
    // needs
    ...
    // TODO: DeltaDB to support output_to_penultimate_level compaction, which
    // needs
    //  2 builders, so may need to move to `CompactionOutputs`
        ...
        (mutable_cf_options->enable_delta_files &&
         sub_compact->compaction->output_level() >=
            ...
            ? new DeltaFileBuilder(
                  ...
                  job_id_, cfd->GetID(), cfd->GetName(),
                  Env::IOPriority::IO_LOW, write_hint_, io_tracer_,
                  delta_callback_, DeltaFileCreationReason::kCompaction,
                  &delta_file_paths,
                  sub_compact->Current().GetDeltaFileAdditionsPtr())
            : nullptr);
    ...
    auto c_iter = std::make_unique<CompactionIterator>(
        ...
        job_snapshot_seq, snapshot_checker_, env_,
    ...
    // Assign range delete aggregator to the target output level, which makes
    // sure it only output to single level
    sub_compact->AssignRangeDelAggregator(std::move(range_del_agg));

    const auto& c_iter_stats = c_iter->iter_stats();

    // define the open and close functions for the compaction files, which will
    // be used open/close output files when needed.
    ...
    while (status.ok() && !cfd->IsDropped() && c_iter->Valid()) {
      ...
      // output file needs to be close or open, it will call the
      // `open_file_func` and `close_file_func`.
      // TODO: it would be better to have the compaction file open/close moved
      // into `CompactionOutputs` which has the output file information.
      status =
          sub_compact->AddToOutput(*c_iter, open_file_func, close_file_func);
      if (!status.ok()) {
        break;
      }
    ...
    sub_compact->compaction_job_stats.num_deltas_read =
        c_iter_stats.num_deltas_read;
    sub_compact->compaction_job_stats.total_delta_bytes_read =
        c_iter_stats.total_delta_bytes_read;
    ...
    if (c_iter_stats.num_deltas_relocated > 0) {
      RecordTick(stats_, DELTA_DB_GC_NUM_KEYS_RELOCATED,
                 c_iter_stats.num_deltas_relocated);
    }
    if (c_iter_stats.total_delta_bytes_relocated > 0) {
      RecordTick(stats_, DELTA_DB_GC_BYTES_RELOCATED,
                 c_iter_stats.total_delta_bytes_relocated);
    }
    ...
    // close the output files. Open file function is also passed, in case
    // there's only range-dels, no file was opened, to save the range-dels, it
    ...
    if (delta_file_builder) {
      if (status.ok()) {
        status = delta_file_builder->Finish();
      } else {
        delta_file_builder->Abandon(status);
      }
      delta_file_builder.reset();
      sub_compact->Current().UpdateDeltaStats();
    }
  ...
  Status CompactionJob::FinishCompactionOutputFile(
    ...
    if (s.ok()) {
      ...
      // output_to_penultimate_level compaction here, as it's only used to
      // decide if range dels could be dropped.
      if (outputs.HasRangeDel()) {
        s = outputs.AddRangeDels(
            sub_compact->start.has_value() ? &(sub_compact->start.value())
                                           : nullptr,
            sub_compact->end.has_value() ? &(sub_compact->end.value())
                                         : nullptr,
    ...
    if (meta != nullptr) {
      ...
      oldest_delta_file_number = meta->oldest_delta_file_number;
    } else {
      fname = "(nil)";
      if (s.ok()) {
    ...
    EventHelpers::LogAndNotifyTableFileCreationFinished(
        ...
        fname, job_id_, output_fd, oldest_blob_file_number,
        oldest_delta_file_number, tp, TableFileCreationReason::kCompaction,
  ...
  Status CompactionJob::InstallCompactionResults(
...

    {
      ...
      if (compaction_stats_.has_penultimate_level_output) {
        ROCKS_LOG_BUFFER(
            log_buffer_,
            "[%s] [JOB %d] Compacted %s => output_to_penultimate_level: "
            "%" PRIu64 " bytes + last: %" PRIu64 " bytes. Total: %" PRIu64
            " bytes",
        ...
        ROCKS_LOG_BUFFER(
            log_buffer_, "[%s] [JOB %d] Compacted %s => %" PRIu64 " bytes",
            compaction->column_family_data()->GetName().c_str(), job_id_,
            compaction->InputLevelSummary(&inputs_summary),
            compaction_stats_.TotalBytesWritten());
      }
    ...
    for (const auto& sub_compact : compact_->sub_compact_states) {
      ...
      if (sub_compact.Current().GetBlobGarbageMeter()) {
        const auto& flows =
            sub_compact.Current().GetBlobGarbageMeter()->flows();
    ...
    std::unordered_map<uint64_t, DeltaGarbageMeter::DeltaStats>
        delta_total_garbage;

    for (const auto& sub_compact : compact_->sub_compact_states) {
      sub_compact.AddOutputsEdit(edit);

      for (const auto& delta : sub_compact.Current().GetDeltaFileAdditions()) {
        edit->AddDeltaFile(delta);
      }

      if (sub_compact.Current().GetDeltaGarbageMeter()) {
        const auto& flows =
            sub_compact.Current().GetDeltaGarbageMeter()->flows();

        for (const auto& pair : flows) {
          const uint64_t delta_file_number = pair.first;
          const DeltaGarbageMeter::DeltaInOutFlow& flow = pair.second;

          assert(flow.IsValid());
          if (flow.HasGarbage()) {
            delta_total_garbage[delta_file_number].Add(flow.GetGarbageCount(),
                                                       flow.GetGarbageBytes());
          }
        }
      }
    ...
    for (const auto& pair : delta_total_garbage) {
      const uint64_t delta_file_number = pair.first;
      const DeltaGarbageMeter::DeltaStats& stats = pair.second;

      edit->AddDeltaFileGarbage(delta_file_number, stats.GetCount(),
                                stats.GetBytes());
    }
    ...
    if (compaction->compaction_reason() ==
      ...
      if (start_level > 0) {
        ...
        edit->AddCompactCursor(
            start_level, vstorage->GetNextCompactCursor(
  ...
  Status CompactionJob::OpenCompactionOutputFile(
    ...
    if (!s.ok()) {
      ...
      EventHelpers::LogAndNotifyTableFileCreationFinished(
          ...
          kInvalidDeltaFileNumber, TableProperties(),
    ...
    auto get_time_status =
        db_options_.clock->GetCurrentTime(&temp_current_time);
  ...
  void CompactionJob::UpdateCompactionStats() {
    ...
    compaction_stats_.stats.bytes_read_delta =
        compaction_job_stats_->total_delta_bytes_read;
  ...
  void CompactionJob::UpdateCompactionJobStats(
    ...
    compaction_job_stats_->total_output_bytes_delta = stats.bytes_written_delta;
    compaction_job_stats_->num_output_records = stats.num_output_records;
    compaction_job_stats_->num_output_files = stats.num_output_files;
    compaction_job_stats_->num_output_files_blob = stats.num_output_files_blob;
    compaction_job_stats_->num_output_files_delta =
        stats.num_output_files_delta;
