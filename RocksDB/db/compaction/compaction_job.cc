...
void CompactionJob::ProcessKeyValueCompaction(SubcompactionState* sub_compact) {
  ...
  if (sub_compact->compaction->DoesInputReferenceDeltaFiles()) {
    DeltaGarbageMeter* meter = sub_compact->Current().CreateDeltaGarbageMeter();
      ...
      (mutable_cf_options->enable_delta_files &&
       sub_compact->compaction->output_level() >=
          ...
          ? new DeltaFileBuilder(
                ...
                mutable_cf_options, &file_options_, db_id_, db_session_id_,
                job_id_, cfd->GetID(), cfd->GetName(), Env::IOPriority::IO_LOW,
                write_hint_, io_tracer_, delta_callback_,
                DeltaFileCreationReason::kCompaction, &delta_file_paths,
...
Status CompactionJob::FinishCompactionOutputFile(
  ...
  EventHelpers::LogAndNotifyTableFileCreationFinished(
      ...
      job_id_, output_fd, oldest_blob_file_number, oldest_delta_file_number, tp,
...
Status CompactionJob::InstallCompactionResults(
  ...
  for (const auto& sub_compact : compact_->sub_compact_states) {
    ...
    if (sub_compact.Current().GetDeltaGarbageMeter()) {
      const auto& flows = sub_compact.Current().GetDeltaGarbageMeter()->flows();
...
void CompactionJob::UpdateCompactionJobStats(
  ...
  compaction_job_stats_->num_output_files_delta = stats.num_output_files_delta;
