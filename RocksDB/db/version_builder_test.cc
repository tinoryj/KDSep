 ...
 public:
  ...
  void AddDummyFile(uint64_t table_file_number, uint64_t blob_file_number,
    ...
    Add(level, table_file_number, smallest, largest, file_size, path_id,
        ...
        smallest_seq, largest_seq, blob_file_number, delta_file_number);
  }
  ...
  void AddDummyFileToEdit(VersionEdit* edit, uint64_t table_file_number,
    ...
    edit->AddFile(
        ...
        delta_file_number, kUnknownOldestAncesterTime, kUnknownFileCreationTime,
        kUnknownFileChecksum, kUnknownFileChecksumFuncName, kNullUniqueId64x2);
  }
...
TEST_F(VersionBuilderTest, ApplyAndSaveTo) {
  ...
  version_edit.AddFile(2, 666, 0, 100U, GetInternalKey("301"),
                       ...
                       kInvalidDeltaFileNumber, kUnknownOldestAncesterTime,
...
TEST_F(VersionBuilderTest, ApplyDeltaFileAddition) {
  UpdateVersionStorageInfo();
  ...
  constexpr uint64_t delta_file_number = 1234;
  constexpr uint64_t total_delta_count = 5678;
  constexpr uint64_t total_delta_bytes = 999999;
  constexpr char checksum_method[] = "SHA1";
  constexpr char checksum_value[] =
      "\xbd\xb7\xf3\x4a\x59\xdf\xa1\x59\x2c\xe7\xf5\x2e\x99\xf9\x8c\x57\x0c\x52"
      "\x5c\xbd";
  ...
  // Add dummy table file to ensure the delta file is referenced.
  constexpr uint64_t table_file_number = 1;
  AddDummyFileToEdit(&edit, table_file_number, delta_file_number);
  ...
  const auto& new_delta_files = new_vstorage.GetDeltaFiles();
  ASSERT_EQ(new_delta_files.size(), 1);

  const auto new_meta = new_vstorage.GetDeltaFileMetaData(delta_file_number);

  ASSERT_NE(new_meta, nullptr);
  ASSERT_EQ(new_meta->GetDeltaFileNumber(), delta_file_number);
  ASSERT_EQ(new_meta->GetTotalDeltaCount(), total_delta_count);
  ASSERT_EQ(new_meta->GetTotalDeltaBytes(), total_delta_bytes);
  ASSERT_EQ(new_meta->GetChecksumMethod(), checksum_method);
  ASSERT_EQ(new_meta->GetChecksumValue(), checksum_value);
  ASSERT_EQ(new_meta->GetLinkedSsts(),
            DeltaFileMetaData::LinkedSsts{table_file_number});
  ASSERT_EQ(new_meta->GetGarbageDeltaCount(), 0);
  ASSERT_EQ(new_meta->GetGarbageDeltaBytes(), 0);

  UnrefFilesInVersion(&new_vstorage);
}

TEST_F(VersionBuilderTest, ApplyDeltaFileAdditionAlreadyInBase) {
  // Attempt to add a delta file that is already present in the base version.
  ...
  constexpr char checksum_value[] =
      "\xbd\xb7\xf3\x4a\x59\xdf\xa1\x59\x2c\xe7\xf5\x2e\x99\xf9\x8c\x57\x0c\x52"
      "\x5c\xbd";
  constexpr uint64_t garbage_delta_count = 123;
  constexpr uint64_t garbage_delta_bytes = 456789;

  AddDelta(delta_file_number, total_delta_count, total_delta_bytes,
           checksum_method, checksum_value, DeltaFileMetaData::LinkedSsts(),
  ...
  const Status s = builder.Apply(&edit);
  ASSERT_TRUE(s.IsCorruption());
  ASSERT_TRUE(std::strstr(s.getState(), "Delta file #1234 already added"));
}

TEST_F(VersionBuilderTest, ApplyDeltaFileAdditionAlreadyApplied) {
  // Attempt to add the same delta file twice using version edits.
...
TEST_F(VersionBuilderTest, ApplyDeltaFileGarbageFileInBase) {
  // Increase the amount of garbage for a delta file present in the base
  // version.
  ...
  const auto meta = vstorage_.GetDeltaFileMetaData(delta_file_number);
  ASSERT_NE(meta, nullptr);

  // Add dummy table file to ensure the delta file is referenced.
  AddDummyFile(table_file_number, delta_file_number);
  ...
  constexpr uint64_t new_garbage_delta_count = 456;
  constexpr uint64_t new_garbage_delta_bytes = 111111;

  edit.AddDeltaFileGarbage(delta_file_number, new_garbage_delta_count,
                           new_garbage_delta_bytes);
  ...
  ASSERT_EQ(new_meta->GetLinkedSsts(),
            DeltaFileMetaData::LinkedSsts{table_file_number});
  ASSERT_EQ(new_meta->GetGarbageDeltaCount(),
            garbage_delta_count + new_garbage_delta_count);
  ASSERT_EQ(new_meta->GetGarbageDeltaBytes(),
            garbage_delta_bytes + new_garbage_delta_bytes);

  UnrefFilesInVersion(&new_vstorage);
}

TEST_F(VersionBuilderTest, ApplyDeltaFileGarbageFileAdditionApplied) {
  // Increase the amount of garbage for a delta file added using a version edit.
  ...
  addition.AddDeltaFile(delta_file_number, total_delta_count, total_delta_bytes,
                        checksum_method, checksum_value);

  // Add dummy table file to ensure the delta file is referenced.
  constexpr uint64_t table_file_number = 1;
  AddDummyFileToEdit(&addition, table_file_number, delta_file_number);
  ...
  garbage.AddDeltaFileGarbage(delta_file_number, garbage_delta_count,
                              garbage_delta_bytes);
  ...
  ASSERT_EQ(new_meta->GetLinkedSsts(),
            DeltaFileMetaData::LinkedSsts{table_file_number});
  ASSERT_EQ(new_meta->GetGarbageDeltaCount(), garbage_delta_count);
  ASSERT_EQ(new_meta->GetGarbageDeltaBytes(), garbage_delta_bytes);

  UnrefFilesInVersion(&new_vstorage);
}

TEST_F(VersionBuilderTest, ApplyDeltaFileGarbageFileNotFound) {
  // Attempt to increase the amount of garbage for a delta file that is
  // neither in the base version, nor was it added using a version edit.
  ...
  constexpr uint64_t delta_file_number = 1234;
  constexpr uint64_t garbage_delta_count = 5678;
  constexpr uint64_t garbage_delta_bytes = 999999;

  edit.AddDeltaFileGarbage(delta_file_number, garbage_delta_count,
                           garbage_delta_bytes);

  const Status s = builder.Apply(&edit);
  ASSERT_TRUE(s.IsCorruption());
  ASSERT_TRUE(std::strstr(s.getState(), "Delta file #1234 not found"));
}

TEST_F(VersionBuilderTest, DeltaFileGarbageOverflow) {
  // Test that VersionEdits that would result in the count/total size of garbage
  // exceeding the count/total size of all deltas are rejected.
  ...
  {
    // Garbage delta count overflow
    constexpr uint64_t garbage_delta_count = 5679;
    constexpr uint64_t garbage_delta_bytes = 999999;
    ...
    ASSERT_TRUE(s.IsCorruption());
    ASSERT_TRUE(
        std::strstr(s.getState(), "Garbage overflow for delta file #1234"));
  }

  {
    // Garbage delta bytes overflow
    constexpr uint64_t garbage_delta_count = 5678;
...
TEST_F(VersionBuilderTest, SaveDeltaFilesTo) {
  // Add three delta files to base version.
  for (uint64_t i = 1; i <= 3; ++i) {
    const uint64_t table_file_number = 2 * i;
    const uint64_t delta_file_number = 2 * i + 1;
    const uint64_t total_delta_count = i * 1000;
    const uint64_t total_delta_bytes = i * 1000000;
    const uint64_t garbage_delta_count = i * 100;
    const uint64_t garbage_delta_bytes = i * 20000;
  ...
  // Add dummy table files to ensure the delta files are referenced.
  // Note: files are added to L0, so they have to be added in reverse order
  // (newest first).
  for (uint64_t i = 3; i >= 1; --i) {
    const uint64_t table_file_number = 2 * i;
  ...
  // Add some garbage to the second and third delta files. The second delta file
  // remains valid since it does not consist entirely of garbage yet. The third
  // delta file is all garbage after the edit and will not be part of the new
  // version. The corresponding dummy table file is also removed for
  // consistency.
  edit.AddDeltaFileGarbage(/* delta_file_number */ 5,
                           /* garbage_delta_count */ 200,
                           /* garbage_delta_bytes */ 100000);
  edit.AddDeltaFileGarbage(/* delta_file_number */ 7,
                           /* garbage_delta_count */ 2700,
                           /* garbage_delta_bytes */ 2940000);
  edit.DeleteFile(/* level */ 0, /* file_number */ 6);

  // Add a fourth delta file.
  edit.AddDeltaFile(/* delta_file_number */ 9, /* total_delta_count */ 4000,
                    /* total_delta_bytes */ 4000000,
                    /* checksum_method */ std::string(),
                    /* checksum_value */ std::string());
  ...
  const auto& new_delta_files = new_vstorage.GetDeltaFiles();
  ASSERT_EQ(new_delta_files.size(), 3);

  const auto meta3 =
      new_vstorage.GetDeltaFileMetaData(/* delta_file_number */ 3);

  ASSERT_NE(meta3, nullptr);
  ASSERT_EQ(meta3->GetDeltaFileNumber(), 3);
  ASSERT_EQ(meta3->GetTotalDeltaCount(), 1000);
  ASSERT_EQ(meta3->GetTotalDeltaBytes(), 1000000);
  ASSERT_EQ(meta3->GetGarbageDeltaCount(), 100);
  ASSERT_EQ(meta3->GetGarbageDeltaBytes(), 20000);

  const auto meta5 =
      new_vstorage.GetDeltaFileMetaData(/* delta_file_number */ 5);

  ASSERT_NE(meta5, nullptr);
  ASSERT_EQ(meta5->GetDeltaFileNumber(), 5);
  ASSERT_EQ(meta5->GetTotalDeltaCount(), 2000);
  ASSERT_EQ(meta5->GetTotalDeltaBytes(), 2000000);
  ASSERT_EQ(meta5->GetGarbageDeltaCount(), 400);
  ASSERT_EQ(meta5->GetGarbageDeltaBytes(), 140000);

  const auto meta9 =
      new_vstorage.GetDeltaFileMetaData(/* delta_file_number */ 9);

  ASSERT_NE(meta9, nullptr);
  ASSERT_EQ(meta9->GetDeltaFileNumber(), 9);
  ASSERT_EQ(meta9->GetTotalDeltaCount(), 4000);
  ASSERT_EQ(meta9->GetTotalDeltaBytes(), 4000000);
  ASSERT_EQ(meta9->GetGarbageDeltaCount(), 0);
  ASSERT_EQ(meta9->GetGarbageDeltaBytes(), 0);

  // Delete the first table file, which makes the first delta file obsolete
  // since it's at the head and unreferenced.
  VersionBuilder second_builder(env_options, &ioptions_, table_cache,
                                &new_vstorage, version_set);
  ...
  const auto& newer_delta_files = newer_vstorage.GetDeltaFiles();
  ASSERT_EQ(newer_delta_files.size(), 2);

  const auto newer_meta3 =
      newer_vstorage.GetDeltaFileMetaData(/* delta_file_number */ 3);
...
TEST_F(VersionBuilderTest, SaveDeltaFilesToConcurrentJobs) {
  // When multiple background jobs (flushes/compactions) are executing in
  // parallel, it is possible for the VersionEdit adding delta file K to be
  // applied *after* the VersionEdit adding delta file N (for N > K). This test
  // case makes sure this is handled correctly.

  // Add delta file #4 (referenced by table file #3) to base version.
  constexpr uint64_t base_table_file_number = 3;
  constexpr uint64_t base_delta_file_number = 4;
  constexpr uint64_t base_total_delta_count = 100;
  constexpr uint64_t base_total_delta_bytes = 1 << 20;
  ...
  AddDummyFile(base_table_file_number, base_delta_file_number);
  AddDelta(base_delta_file_number, base_total_delta_count,
           base_total_delta_bytes, checksum_method, checksum_value,
           DeltaFileMetaData::LinkedSsts{base_table_file_number},
           garbage_delta_count, garbage_delta_bytes);
  ...
  // Add delta file #2 (referenced by table file #1).
  constexpr int level = 0;
  constexpr uint64_t table_file_number = 1;
  constexpr uint32_t path_id = 0;
  constexpr uint64_t file_size = 1 << 12;
  ...
  static_assert(delta_file_number < base_delta_file_number,
                "Added delta file should have a smaller file number");

  constexpr uint64_t total_delta_count = 234;
  constexpr uint64_t total_delta_bytes = 1 << 22;

  edit.AddFile(level, table_file_number, path_id, file_size,
               GetInternalKey(smallest), GetInternalKey(largest),
               smallest_seqno, largest_seqno, marked_for_compaction,
               Temperature::kUnknown, delta_file_number,
               kUnknownOldestAncesterTime, kUnknownFileCreationTime,
               checksum_value, checksum_method, kNullUniqueId64x2);
  edit.AddDeltaFile(delta_file_number, total_delta_count, total_delta_bytes,
                    checksum_method, checksum_value);
  ...
  const auto& new_delta_files = new_vstorage.GetDeltaFiles();
  ASSERT_EQ(new_delta_files.size(), 2);

  const auto base_meta =
      new_vstorage.GetDeltaFileMetaData(base_delta_file_number);

  ASSERT_NE(base_meta, nullptr);
  ASSERT_EQ(base_meta->GetDeltaFileNumber(), base_delta_file_number);
  ASSERT_EQ(base_meta->GetTotalDeltaCount(), base_total_delta_count);
  ASSERT_EQ(base_meta->GetTotalDeltaBytes(), base_total_delta_bytes);
  ASSERT_EQ(base_meta->GetGarbageDeltaCount(), garbage_delta_count);
  ASSERT_EQ(base_meta->GetGarbageDeltaBytes(), garbage_delta_bytes);
  ASSERT_EQ(base_meta->GetChecksumMethod(), checksum_method);
  ASSERT_EQ(base_meta->GetChecksumValue(), checksum_value);

  const auto added_meta = new_vstorage.GetDeltaFileMetaData(delta_file_number);

  ASSERT_NE(added_meta, nullptr);
  ASSERT_EQ(added_meta->GetDeltaFileNumber(), delta_file_number);
  ASSERT_EQ(added_meta->GetTotalDeltaCount(), total_delta_count);
  ASSERT_EQ(added_meta->GetTotalDeltaBytes(), total_delta_bytes);
  ASSERT_EQ(added_meta->GetGarbageDeltaCount(), garbage_delta_count);
  ASSERT_EQ(added_meta->GetGarbageDeltaBytes(), garbage_delta_bytes);
  ASSERT_EQ(added_meta->GetChecksumMethod(), checksum_method);
  ASSERT_EQ(added_meta->GetChecksumValue(), checksum_value);
...
TEST_F(VersionBuilderTest, CheckConsistencyForDeltaFiles) {
  // Initialize base version. The first table file points to a valid delta file
  // in this version; the second one does not refer to any delta files.
  ...
  AddDelta(/* delta_file_number */ 16, /* total_delta_count */ 1000,
           /* total_delta_bytes */ 1000000,
           /* checksum_method */ std::string(),
           /* checksum_value */ std::string(), DeltaFileMetaData::LinkedSsts{1},
           /* garbage_delta_count */ 500, /* garbage_delta_bytes */ 300000);

  UpdateVersionStorageInfo();

  // Add a new table file that points to the existing delta file, and add a
  // new table file--delta file pair.
  EnvOptions env_options;
  constexpr TableCache* table_cache = nullptr;
  constexpr VersionSet* version_set = nullptr;
  ...
  edit.AddFile(/* level */ 1, /* file_number */ 606, /* path_id */ 0,
               ...
               /* largest_seqno */ 200, /* marked_for_compaction */ false,
               Temperature::kUnknown,
               /* oldest_delta_file_number */ 16, kUnknownOldestAncesterTime,
               kUnknownFileCreationTime, kUnknownFileChecksum,
               kUnknownFileChecksumFuncName, kNullUniqueId64x2);
  ...
  edit.AddFile(/* level */ 1, /* file_number */ 700, /* path_id */ 0,
               ...
               /* largest_seqno */ 200, /* marked_for_compaction */ false,
               Temperature::kUnknown,
               /* oldest_delta_file_number */ 1000, kUnknownOldestAncesterTime,
               kUnknownFileCreationTime, kUnknownFileChecksum,
               kUnknownFileChecksumFuncName, kNullUniqueId64x2);
  edit.AddDeltaFile(/* delta_file_number */ 1000, /* total_delta_count */ 2000,
                    /* total_delta_bytes */ 200000,
                    /* checksum_method */ std::string(),
                    /* checksum_value */ std::string());
...
TEST_F(VersionBuilderTest, CheckConsistencyForDeltaFilesInconsistentLinks) {
  // Initialize base version. Links between the table file and the delta file
  // are inconsistent.
  ...
  ASSERT_TRUE(std::strstr(
      s.getState(),
      "Links are inconsistent between table files and delta file #16"));

  UnrefFilesInVersion(&new_vstorage);
}

TEST_F(VersionBuilderTest, CheckConsistencyForDeltaFilesAllGarbage) {
  // Initialize base version. The table file points to a delta file that is
  // all garbage.
  ...
  AddDelta(/* delta_file_number */ 16, /* total_delta_count */ 1000,
           ...
           /* checksum_method */ std::string(),
           /* checksum_value */ std::string(), DeltaFileMetaData::LinkedSsts{1},
           /* garbage_delta_count */ 1000, /* garbage_delta_bytes */ 1000000);
  ...
  ASSERT_TRUE(s.IsCorruption());
  ASSERT_TRUE(
      std::strstr(s.getState(), "Delta file #16 consists entirely of garbage"));

  UnrefFilesInVersion(&new_vstorage);
}

TEST_F(VersionBuilderTest, CheckConsistencyForDeltaFilesAllGarbageLinkedSsts) {
  // Initialize base version, with a table file pointing to a delta file
  // that has no garbage at this point.
  ...
  AddDelta(/* delta_file_number */ 16, /* total_delta_count */ 1000,
           ...
           /* checksum_method */ std::string(),
           /* checksum_value */ std::string(), DeltaFileMetaData::LinkedSsts{1},
           /* garbage_delta_count */ 0, /* garbage_delta_bytes */ 0);

  UpdateVersionStorageInfo();

  // Mark the entire delta file garbage but do not remove the linked SST.
  EnvOptions env_options;
  constexpr TableCache* table_cache = nullptr;
  constexpr VersionSet* version_set = nullptr;
  ...
  edit.AddDeltaFileGarbage(/* delta_file_number */ 16,
                           /* garbage_delta_count */ 1000,
                           /* garbage_delta_bytes */ 1000000);
...
TEST_F(VersionBuilderTest, MaintainLinkedSstsForDeltaFiles) {
  // Initialize base version. Table files 1..10 are linked to delta files 1..5,
  // while table files 11..20 are not linked to any delta files.
  ...
  for (uint64_t i = 1; i <= 10; ++i) {
    ...
    Add(/* level */ 1, /* file_number */ i, /* smallest */ key.c_str(),
        ...
        /* sampled */ false, /* smallest_seqno */ i * 100,
        /* largest_seqno */ i * 100,
        /* oldest_delta_file_number */ ((i - 1) % 5) + 1);
  }

  for (uint64_t i = 1; i <= 5; ++i) {
    AddDelta(/* delta_file_number */ i, /* total_delta_count */ 2000,
             /* total_delta_bytes */ 2000000,
             /* checksum_method */ std::string(),
             /* checksum_value */ std::string(),
             DeltaFileMetaData::LinkedSsts{i, i + 5},
             /* garbage_delta_count */ 1000, /* garbage_delta_bytes */ 1000000);
  }
  ...
  for (uint64_t i = 11; i <= 20; ++i) {
    ...
    Add(/* level */ 1, /* file_number */ i, /* smallest */ key.c_str(),
        ...
        /* num_entries */ 0, /* num_deletions */ 0,
        /* sampled */ false, /* smallest_seqno */ i * 100,
        /* largest_seqno */ i * 100, kInvalidDeltaFileNumber);
  }

  UpdateVersionStorageInfo();

  {
    const auto& delta_files = vstorage_.GetDeltaFiles();
    ASSERT_EQ(delta_files.size(), 5);

    const std::vector<DeltaFileMetaData::LinkedSsts> expected_linked_ssts{
        {1, 6}, {2, 7}, {3, 8}, {4, 9}, {5, 10}};

    for (size_t i = 0; i < 5; ++i) {
      const auto meta =
          vstorage_.GetDeltaFileMetaData(/* delta_file_number */ i + 1);
      ASSERT_NE(meta, nullptr);
      ASSERT_EQ(meta->GetLinkedSsts(), expected_linked_ssts[i]);
    }
  }
  ...
  // Add an SST that references a delta file.
  edit.AddFile(
      /* level */ 1, /* file_number */ 21, /* path_id */ 0,
      /* file_size */ 100, /* smallest */ GetInternalKey("21", 2100),
      /* largest */ GetInternalKey("21", 2100), /* smallest_seqno */ 2100,
      /* largest_seqno */ 2100, /* marked_for_compaction */ false,
      Temperature::kUnknown,
      /* oldest_delta_file_number */ 1, kUnknownOldestAncesterTime,
      kUnknownFileCreationTime, kUnknownFileChecksum,
      kUnknownFileChecksumFuncName, kNullUniqueId64x2);

  // Add an SST that does not reference any delta files.
  edit.AddFile(
      /* level */ 1, /* file_number */ 22, /* path_id */ 0,
      /* file_size */ 100, /* smallest */ GetInternalKey("22", 2200),
      /* largest */ GetInternalKey("22", 2200), /* smallest_seqno */ 2200,
      /* largest_seqno */ 2200, /* marked_for_compaction */ false,
      Temperature::kUnknown, kInvalidDeltaFileNumber,
      kUnknownOldestAncesterTime, kUnknownFileCreationTime,
      kUnknownFileChecksum, kUnknownFileChecksumFuncName, kNullUniqueId64x2);

  // Delete a file that references a delta file.
  edit.DeleteFile(/* level */ 1, /* file_number */ 6);

  // Delete a file that does not reference any delta files.
  edit.DeleteFile(/* level */ 1, /* file_number */ 16);

  // Trivially move a file that references a delta file. Note that we save
  // the original DeltaFileMetaData object so we can check that no new object
  // gets created.
  auto meta3 = vstorage_.GetDeltaFileMetaData(/* delta_file_number */ 3);
  ...
  edit.AddFile(/* level */ 2, /* file_number */ 3, /* path_id */ 0,
               ...
               /* largest_seqno */ 300, /* marked_for_compaction */ false,
               Temperature::kUnknown,
               /* oldest_delta_file_number */ 3, kUnknownOldestAncesterTime,
               kUnknownFileCreationTime, kUnknownFileChecksum,
               kUnknownFileChecksumFuncName, kNullUniqueId64x2);

  // Trivially move a file that does not reference any delta files.
  edit.DeleteFile(/* level */ 1, /* file_number */ 13);
  edit.AddFile(/* level */ 2, /* file_number */ 13, /* path_id */ 0,
               /* file_size */ 100, /* smallest */ GetInternalKey("13", 1300),
               /* largest */ GetInternalKey("13", 1300),
  ...
  // Add one more SST file that references a delta file, then promptly
  // delete it in a second version edit before the new version gets saved.
  // This file should not show up as linked to the delta file in the new
  // version.
  edit.AddFile(/* level */ 1, /* file_number */ 23, /* path_id */ 0,
               /* file_size */ 100, /* smallest */ GetInternalKey("23", 2300),
               /* largest */ GetInternalKey("23", 2300),
               ...
               /* largest_seqno */ 2300, /* marked_for_compaction */ false,
               Temperature::kUnknown,
               /* oldest_delta_file_number */ 5, kUnknownOldestAncesterTime,
               kUnknownFileCreationTime, kUnknownFileChecksum,
               kUnknownFileChecksumFuncName, kNullUniqueId64x2);
  ...
  {
    const auto& delta_files = new_vstorage.GetDeltaFiles();
    ASSERT_EQ(delta_files.size(), 5);
    ...
    for (size_t i = 0; i < 5; ++i) {
      const auto meta =
          new_vstorage.GetDeltaFileMetaData(/* delta_file_number */ i + 1);
      ASSERT_NE(meta, nullptr);
      ASSERT_EQ(meta->GetLinkedSsts(), expected_linked_ssts[i]);
    }

    // Make sure that no new DeltaFileMetaData got created for the delta file
    // affected by the trivial move.
    ASSERT_EQ(new_vstorage.GetDeltaFileMetaData(/* delta_file_number */ 3),
              meta3);
  }
