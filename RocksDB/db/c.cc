...
using ROCKSDB_NAMESPACE::PrepopulateDeltaCache;
...
/* Delta Options Settings */
void rocksdb_options_set_enable_delta_files(rocksdb_options_t* opt,
                                            unsigned char val) {
  opt->rep.enable_delta_files = val;
}
extern ROCKSDB_LIBRARY_API unsigned char rocksdb_options_get_enable_delta_files(
    rocksdb_options_t* opt) {
  return opt->rep.enable_delta_files;
}

void rocksdb_options_set_min_delta_size(rocksdb_options_t* opt, uint64_t val) {
  opt->rep.min_delta_size = val;
}

uint64_t rocksdb_options_get_min_delta_size(rocksdb_options_t* opt) {
  return opt->rep.min_delta_size;
}

void rocksdb_options_set_delta_file_size(rocksdb_options_t* opt, uint64_t val) {
  opt->rep.delta_file_size = val;
}

uint64_t rocksdb_options_get_delta_file_size(rocksdb_options_t* opt) {
  return opt->rep.delta_file_size;
}

void rocksdb_options_set_delta_compression_type(rocksdb_options_t* opt,
                                                int val) {
  opt->rep.delta_compression_type = static_cast<CompressionType>(val);
}

int rocksdb_options_get_delta_compression_type(rocksdb_options_t* opt) {
  return opt->rep.delta_compression_type;
}

void rocksdb_options_set_enable_delta_gc(rocksdb_options_t* opt,
                                         unsigned char val) {
  opt->rep.enable_delta_garbage_collection = val;
}

unsigned char rocksdb_options_get_enable_delta_gc(rocksdb_options_t* opt) {
  return opt->rep.enable_delta_garbage_collection;
}

void rocksdb_options_set_delta_gc_age_cutoff(rocksdb_options_t* opt,
                                             double val) {
  opt->rep.delta_garbage_collection_age_cutoff = val;
}

double rocksdb_options_get_delta_gc_age_cutoff(rocksdb_options_t* opt) {
  return opt->rep.delta_garbage_collection_age_cutoff;
}

void rocksdb_options_set_delta_gc_force_threshold(rocksdb_options_t* opt,
                                                  double val) {
  opt->rep.delta_garbage_collection_force_threshold = val;
}

double rocksdb_options_get_delta_gc_force_threshold(rocksdb_options_t* opt) {
  return opt->rep.delta_garbage_collection_force_threshold;
}

void rocksdb_options_set_delta_compaction_readahead_size(rocksdb_options_t* opt,
                                                         uint64_t val) {
  opt->rep.delta_compaction_readahead_size = val;
}

uint64_t rocksdb_options_get_delta_compaction_readahead_size(
    rocksdb_options_t* opt) {
  return opt->rep.delta_compaction_readahead_size;
}

void rocksdb_options_set_delta_file_starting_level(rocksdb_options_t* opt,
                                                   int val) {
  opt->rep.delta_file_starting_level = val;
}

int rocksdb_options_get_delta_file_starting_level(rocksdb_options_t* opt) {
  return opt->rep.delta_file_starting_level;
}

void rocksdb_options_set_delta_cache(rocksdb_options_t* opt,
                                     rocksdb_cache_t* delta_cache) {
  opt->rep.delta_cache = delta_cache->rep;
}

void rocksdb_options_set_prepopulate_delta_cache(rocksdb_options_t* opt,
                                                 int t) {
  opt->rep.prepopulate_delta_cache = static_cast<PrepopulateDeltaCache>(t);
}

int rocksdb_options_get_prepopulate_delta_cache(rocksdb_options_t* opt) {
  return static_cast<int>(opt->rep.prepopulate_delta_cache);
}
...
uint64_t rocksdb_perfcontext_metric(rocksdb_perfcontext_t* context,
  ...
  switch (metric) {
    ...
    case rocksdb_delta_cache_hit_count:
      return rep->delta_cache_hit_count;
    case rocksdb_delta_read_count:
      return rep->delta_read_count;
    case rocksdb_delta_read_byte:
      return rep->delta_read_byte;
    case rocksdb_delta_read_time:
      return rep->delta_read_time;
    case rocksdb_delta_checksum_time:
      return rep->delta_checksum_time;
    case rocksdb_delta_decompress_time:
      return rep->delta_decompress_time;
