...
class DeltaFileCache {
 public:
  DeltaFileCache(Cache* cache, const ImmutableOptions* immutable_options,
                 const FileOptions* file_options, uint32_t column_family_id,
                 HistogramImpl* delta_file_read_hist,
                 const std::shared_ptr<IOTracer>& io_tracer);

  DeltaFileCache(const DeltaFileCache&) = delete;
  DeltaFileCache& operator=(const DeltaFileCache&) = delete;

  Status GetDeltaFileReader(
 ...
 private:
  ...
  HistogramImpl* delta_file_read_hist_;
