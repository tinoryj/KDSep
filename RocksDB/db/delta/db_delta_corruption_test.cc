...
class DBDeltaCorruptionTest : public DBTestBase {
 protected:
  DBDeltaCorruptionTest()
      : DBTestBase("db_delta_corruption_test", /* env_do_fsync */ false) {}
  ...
  void Corrupt(FileType filetype, int offset, int bytes_to_corrupt) {
    ...
    FileType type;
    std::string fname;
    uint64_t picked_number = kInvalidDeltaFileNumber;
...
#ifndef ROCKSDB_LITE
TEST_F(DBDeltaCorruptionTest, VerifyWholeDeltaFileChecksum) {
  ...
  ASSERT_OK(Put(Slice("key_1"), Slice("delta_value_1")));
  ASSERT_OK(Flush());
  ASSERT_OK(Put(Slice("key_2"), Slice("delta_value_2")));
  ASSERT_OK(Flush());
  ASSERT_OK(db_->VerifyFileChecksums(ReadOptions()));
  Close();

  Corrupt(kDeltaFile, 0, 2);
