...
TEST(DeltaGarbageMeterTest, MeasureGarbage) {
  ...
  struct DeltaDescriptor {
    ...
    uint64_t GetExpectedBytes() const {
      return size + DeltaLogRecord::CalculateAdjustmentForRecordHeader(
                        user_key.size());
    }
  };

  // Note: delta file 4 has the same inflow and outflow and hence no additional
  // garbage. Delta file 5 has less outflow than inflow and thus it does have
  // additional garbage. Delta file 6 is a newly written file (i.e. no inflow,
  // only outflow) and is thus not tracked by the meter.
  std::vector<DeltaDescriptor> deltas{
  ...
  for (const auto& delta : deltas) {
    ...
    const InternalKey key(delta.user_key, seq, kTypeDeltaIndex);
    const Slice key_slice = key.Encode();

    std::string value;
    DeltaIndex::EncodeDelta(&value, delta.delta_file_number, delta.offset,
                            delta.size, delta.compression_type);
    const Slice value_slice(value);

    if (delta.has_in_flow) {
      ASSERT_OK(delta_garbage_meter.ProcessInFlow(key_slice, value_slice));
    }
    if (delta.has_out_flow) {
      ASSERT_OK(delta_garbage_meter.ProcessOutFlow(key_slice, value_slice));
    }
  }
...

  {
    ...
    const uint64_t expected_bytes =
        deltas[0].GetExpectedBytes() + deltas[1].GetExpectedBytes();
...

  {
    ...
    const uint64_t expected_in_bytes =
        deltas[2].GetExpectedBytes() + deltas[3].GetExpectedBytes() +
        deltas[4].GetExpectedBytes() + deltas[5].GetExpectedBytes();
...
TEST(DeltaGarbageMeterTest, PlainValue) {
  ...
  ASSERT_OK(delta_garbage_meter.ProcessInFlow(key_slice, value_slice));
  ASSERT_OK(delta_garbage_meter.ProcessOutFlow(key_slice, value_slice));
  ASSERT_TRUE(delta_garbage_meter.flows().empty());
}

TEST(DeltaGarbageMeterTest, CorruptInternalKey) {
  ...
  ASSERT_NOK(delta_garbage_meter.ProcessInFlow(key_slice, value_slice));
  ASSERT_NOK(delta_garbage_meter.ProcessOutFlow(key_slice, value_slice));
}

TEST(DeltaGarbageMeterTest, CorruptDeltaIndex) {
  constexpr char user_key[] = "user_key";
  constexpr SequenceNumber seq = 123;

  const InternalKey key(user_key, seq, kTypeDeltaIndex);
  const Slice key_slice = key.Encode();

  constexpr char value[] = "i_am_not_a_delta_index";
  const Slice value_slice(value);
...
TEST(DeltaGarbageMeterTest, InlinedTTLDeltaIndex) {
  constexpr char user_key[] = "user_key";
  constexpr SequenceNumber seq = 123;
  ...
  std::string value;
  DeltaIndex::EncodeInlinedTTL(&value, expiration, inlined_value);
