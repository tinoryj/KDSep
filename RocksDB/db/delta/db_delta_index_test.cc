...
// kTypeDeltaIndex is a value type used by DeltaDB only. The base rocksdb
...
// doesn't understand format of actual delta index (the value).
class DBDeltaIndexTest : public DBTestBase {
 public:
  enum Tier {
    kMemtable = 0,
  ...
  DBDeltaIndexTest()
      : DBTestBase("db_delta_index_test", /*env_do_fsync=*/true) {}
  ...
  Status PutDeltaIndex(WriteBatch* batch, const Slice& key,
                       const Slice& delta_index) {
    return WriteBatchInternal::PutDeltaIndex(batch, cfd()->GetID(), key,
                                             delta_index);
  }
  ...
  std::string GetImpl(const Slice& key, bool* is_delta_index = nullptr,
  ...
  std::string GetDeltaIndex(const Slice& key,
                            const Snapshot* snapshot = nullptr) {
    bool is_delta_index = false;
    std::string value = GetImpl(key, &is_delta_index, snapshot);
    if (!is_delta_index) {
      return "NOT_DELTA";
    }
    return value;
  ...
  ArenaWrappedDBIter* GetDeltaIterator() {
...
// Note: the following test case pertains to the StackableDB-based DeltaDB
// implementation. We should be able to write kTypeDeltaIndex to memtables and
// SST files.
TEST_F(DBDeltaIndexTest, Write) {
  for (auto tier : kAllTiers) {
    ...
    for (size_t i = 1; i <= num_key_values; ++i) {
      ...
      DeltaIndex::EncodeInlinedTTL(&delta_index, /* expiration */ 9876543210,
                                   "delta" + std::to_string(i));

      key_values.emplace_back(std::move(key), std::move(delta_index));
    }

    for (const auto& key_value : key_values) {
      WriteBatch batch;
      ASSERT_OK(PutDeltaIndex(&batch, key_value.first, key_value.second));
    ...
    for (const auto& key_value : key_values) {
      ASSERT_EQ(GetDeltaIndex(key_value.first), key_value.second);
    }
  }
}

// Note: the following test case pertains to the StackableDB-based DeltaDB
// implementation. Get should be able to return delta index if is_delta_index is
...
// accidentally opening the base DB of a stacked DeltaDB and actual corruption
// when using the integrated DeltaDB.
TEST_F(DBDeltaIndexTest, Get) {
  std::string delta_index;
  DeltaIndex::EncodeInlinedTTL(&delta_index, /* expiration */ 9876543210,
                               "delta");
  ...
  for (auto tier : kAllTiers) {
    ...
    ASSERT_OK(PutDeltaIndex(&batch, "delta_key", delta_index));
    ASSERT_OK(Write(&batch));
    ...
    ASSERT_EQ("value", GetImpl("key", &is_delta_index));
    ASSERT_FALSE(is_delta_index);

    // Verify delta index
    if (tier <= kImmutableMemtables) {
      ASSERT_TRUE(Get("delta_key", &value).IsNotSupported());
      ASSERT_EQ("NOT_SUPPORTED", GetImpl("delta_key"));
    } else {
      ASSERT_TRUE(Get("delta_key", &value).IsCorruption());
      ASSERT_EQ("CORRUPTION", GetImpl("delta_key"));
    }
    ASSERT_EQ(delta_index, GetImpl("delta_key", &is_delta_index));
    ASSERT_TRUE(is_delta_index);
  }
}

// Note: the following test case pertains to the StackableDB-based DeltaDB
// implementation. Get should NOT return Status::NotSupported/Status::Corruption
// if delta index is updated with a normal value. See the test case above for
// more details.
TEST_F(DBDeltaIndexTest, Updated) {
  std::string delta_index;
  DeltaIndex::EncodeInlinedTTL(&delta_index, /* expiration */ 9876543210,
                               "delta");
  ...
  for (auto tier : kAllTiers) {
    ...
    WriteBatch batch;
    for (int i = 0; i < 10; i++) {
      ASSERT_OK(PutDeltaIndex(&batch, "key" + std::to_string(i), delta_index));
    }
    ASSERT_OK(Write(&batch));
    // Avoid delta values from being purged.
    ...
    MoveDataTo(tier);
    for (int i = 0; i < 10; i++) {
      ASSERT_EQ(delta_index,
                GetDeltaIndex("key" + std::to_string(i), snapshot));
    }
    ...
    ASSERT_EQ(delta_index, GetDeltaIndex("key9"));
    dbfull()->ReleaseSnapshot(snapshot);
  }
}

// Note: the following test case pertains to the StackableDB-based DeltaDB
// implementation. When a delta iterator is used, it should set the
// expose_delta_index flag for the underlying DBIter, and retrieve/return the
// corresponding delta value. If a regular DBIter is created (i.e.
// expose_delta_index is not set), it should return Status::Corruption.
TEST_F(DBDeltaIndexTest, Iterate) {
  const std::vector<std::vector<ValueType>> data = {
      /*00*/ {kTypeValue},
      /*01*/ {kTypeDeltaIndex},
      /*02*/ {kTypeValue},
      /*03*/ {kTypeDeltaIndex, kTypeValue},
      /*04*/ {kTypeValue},
      /*05*/ {kTypeValue, kTypeDeltaIndex},
      /*06*/ {kTypeValue},
      /*07*/ {kTypeDeletion, kTypeDeltaIndex},
      /*08*/ {kTypeValue},
      /*09*/ {kTypeSingleDeletion, kTypeDeltaIndex},
      /*10*/ {kTypeValue},
      /*11*/ {kTypeMerge, kTypeMerge, kTypeMerge, kTypeDeltaIndex},
      /*12*/ {kTypeValue},
      /*13*/
      {kTypeMerge, kTypeMerge, kTypeMerge, kTypeDeletion, kTypeDeltaIndex},
      /*14*/ {kTypeValue},
      /*15*/ {kTypeDeltaIndex},
      /*16*/ {kTypeValue},
  };
  ...
  auto create_delta_iterator = [&]() -> Iterator* {
    return GetDeltaIterator();
  };

  auto check_is_delta = [&](bool is_delta) {
    return [is_delta](Iterator* iterator) {
      ASSERT_EQ(is_delta,
                reinterpret_cast<ArenaWrappedDBIter*>(iterator)->IsDelta());
    };
  };
  ...
  for (auto tier : {Tier::kMemtable} /*kAllTiers*/) {
      ...
      for (int j = static_cast<int>(data[i].size()) - 1; j >= 0; j--) {
        ...
        switch (data[i][j]) {
          ...
          case kTypeDeltaIndex:
            ASSERT_OK(PutDeltaIndex(&batch, key, value));
            ASSERT_OK(Write(&batch));
            break;
          default:
            FAIL();
    ...
    // Iterator with delta support
    verify(1, Status::kOk, get_value(1, 0), get_value(1, 0),
           create_delta_iterator, check_is_delta(true));
    verify(3, Status::kOk, get_value(3, 0), get_value(3, 0),
           create_delta_iterator, check_is_delta(true));
    verify(5, Status::kOk, get_value(5, 0), get_value(5, 0),
           create_delta_iterator, check_is_delta(false));
    verify(7, Status::kOk, get_value(8, 0), get_value(6, 0),
           create_delta_iterator, check_is_delta(false));
    verify(9, Status::kOk, get_value(10, 0), get_value(8, 0),
           create_delta_iterator, check_is_delta(false));
    if (tier <= kImmutableMemtables) {
      verify(11, Status::kNotSupported, "", "", create_delta_iterator);
    } else {
      verify(11, Status::kCorruption, "", "", create_delta_iterator);
    }
    verify(13, Status::kOk,
           get_value(13, 2) + "," + get_value(13, 1) + "," + get_value(13, 0),
           get_value(13, 2) + "," + get_value(13, 1) + "," + get_value(13, 0),
...
#ifndef ROCKSDB_LITE
    // Iterator with delta support and using seek.
...
TEST_F(DBDeltaIndexTest, IntegratedDeltaIterate) {
