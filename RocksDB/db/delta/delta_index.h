...
// DeltaIndex is a pointer to the delta and metadata of the delta. The index is
// stored in base DB as ValueType::kTypeDeltaIndex.
// There are three types of delta index:
//
//    kInlinedTTL:
//      +------+------------+---------------+
...
class DeltaIndex {
 public:
  ...
  DeltaIndex() : type_(Type::kUnknown) {}

  DeltaIndex(const DeltaIndex&) = default;
  DeltaIndex& operator=(const DeltaIndex&) = default;

  bool IsInlined() const { return type_ == Type::kInlinedTTL; }

  bool HasTTL() const {
    return type_ == Type::kInlinedTTL || type_ == Type::kDeltaTTL;
  }
  ...
  Status DecodeFrom(Slice slice) {
    ...
    if (IsInlined()) {
      ...
      if (GetVarint64(&slice, &file_number_) && GetVarint64(&slice, &offset_) &&
        ...
        return Status::Corruption(kErrorMessage, "Corrupted delta offset");
      }
    }
    return Status::OK();
  }
  ...
  std::string DebugString(bool output_hex) const {
    ...
    if (IsInlined()) {
      oss << "[inlined delta] value:" << value_.ToString(output_hex);
    } else {
      oss << "[delta ref] file:" << file_number_ << " offset:" << offset_
          << " size:" << size_
  ...
  static void EncodeDelta(std::string* dst, uint64_t file_number,
                          uint64_t offset, uint64_t size,
                          CompressionType compression) {
    assert(dst != nullptr);
    dst->clear();
    dst->reserve(kMaxVarint64Length * 3 + 2);
    dst->push_back(static_cast<char>(Type::kDelta));
