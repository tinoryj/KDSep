...
DeltaLogWriter::DeltaLogWriter(std::unique_ptr<WritableFileWriter>&& dest,
...
Status DeltaLogWriter::Sync() {
  TEST_SYNC_POINT("DeltaLogWriter::Sync");

  StopWatch sync_sw(clock_, statistics_, DELTA_DB_DELTA_FILE_SYNC_MICROS);
  Status s = dest_->Sync(use_fsync_);
  RecordTick(statistics_, DELTA_DB_DELTA_FILE_SYNCED);
  return s;
}

Status DeltaLogWriter::WriteHeader(DeltaLogHeader& header) {
  ...
  RecordTick(statistics_, DELTA_DB_DELTA_FILE_BYTES_WRITTEN,
             DeltaLogHeader::kSize);
  return s;
}

Status DeltaLogWriter::AppendFooter(DeltaLogFooter& footer,
  ...
  RecordTick(statistics_, DELTA_DB_DELTA_FILE_BYTES_WRITTEN,
             DeltaLogFooter::kSize);
  return s;
}

Status DeltaLogWriter::AddRecord(const Slice& key, const Slice& val,
  ...
  std::string buf;
  ConstructDeltaHeader(&buf, key, val, expiration);

  Status s = EmitPhysicalRecord(buf, key, val, key_offset, delta_offset);
  return s;
}
...
Status DeltaLogWriter::AddRecord(const Slice& key, const Slice& val,
  ...
  std::string buf;
  ConstructDeltaHeader(&buf, key, val, 0);

  Status s = EmitPhysicalRecord(buf, key, val, key_offset, delta_offset);
  return s;
}

void DeltaLogWriter::ConstructDeltaHeader(std::string* buf, const Slice& key,
                                          const Slice& val,
                                          uint64_t expiration) {
...
Status DeltaLogWriter::EmitPhysicalRecord(const std::string& headerbuf,
                                          const Slice& key, const Slice& val,
                                          uint64_t* key_offset,
                                          uint64_t* delta_offset) {
  StopWatch write_sw(clock_, statistics_, DELTA_DB_DELTA_FILE_WRITE_MICROS);
  ...
  *key_offset = block_offset_ + DeltaLogRecord::kHeaderSize;
  *delta_offset = *key_offset + key.size();
  block_offset_ = *delta_offset + val.size();
  last_elem_type_ = kEtRecord;
  RecordTick(statistics_, DELTA_DB_DELTA_FILE_BYTES_WRITTEN,
             DeltaLogRecord::kHeaderSize + key.size() + val.size());
  return s;
}
