...
class DeltaFileCompletionCallback {
 public:
  ...
  void OnDeltaFileCreationStarted(const std::string& file_name,
                                  const std::string& column_family_name,
                                  int job_id,
                                  DeltaFileCreationReason creation_reason) {
#ifndef ROCKSDB_LITE
    // Notify the listeners.
    EventHelpers::NotifyDeltaFileCreationStarted(listeners_, dbname_,
  ...
  Status OnDeltaFileCompleted(const std::string& file_name,
                              ...
                              uint64_t delta_count, uint64_t delta_bytes) {
    Status s;

#ifndef ROCKSDB_LITE
    auto sfm = static_cast<SstFileManagerImpl*>(sst_file_manager_);
    if (sfm) {
      // Report new delta files to SstFileManagerImpl
    ...
    // Notify the listeners.
    EventHelpers::LogAndNotifyDeltaFileCreationFinished(
