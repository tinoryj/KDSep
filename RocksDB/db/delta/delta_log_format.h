...
// Format of delta log file header (30 bytes):
//
//    +--------------+---------+---------+-------+-------------+-------------------+
...
// delta_db_options.ttl_range_secs.
struct DeltaLogHeader {
  static constexpr size_t kSize = 30;

  DeltaLogHeader() = default;
  DeltaLogHeader(uint32_t _column_family_id, CompressionType _compression,
...
// Format of delta log file footer (32 bytes):
//
//    +--------------+------------+-------------------+------------+
//    | magic number | delta count | expiration range  | footer CRC |
//    +--------------+------------+-------------------+------------+
//    |   Fixed32    |  Fixed64   | Fixed64 + Fixed64 |   Fixed32  |
//    +--------------+------------+-------------------+------------+
//
// The footer will be presented only when the delta file is properly closed.
//
...
// Delta record format (32 bytes header + key + value):
//
//    +------------+--------------+------------+------------+----------+---------+-----------+
//    | key length | value length | expiration | header CRC | delta CRC |   key
//    |   value   |
//    +------------+--------------+------------+------------+----------+---------+-----------+
...
//    +------------+--------------+------------+------------+----------+---------+-----------+
//
// If file has has_ttl = false, expiration field is always 0, and the delta
...
// delta CRC is the checksum of (key + value).
//
...
struct DeltaLogRecord {
  // header include fields up to delta CRC
  static constexpr size_t kHeaderSize = 32;

  // Note that the offset field of DeltaIndex actually points to the delta value
  // as opposed to the start of the delta record. The following method can
  // be used to calculate the adjustment needed to read the delta record header.
  ...
  Status CheckDeltaCRC() const;
};

// Checks whether a delta offset is potentially valid or not.
inline bool IsValidDeltaOffset(uint64_t value_offset, uint64_t key_size,
                               uint64_t value_size, uint64_t file_size) {
  if (value_offset <
      DeltaLogHeader::kSize + DeltaLogRecord::kHeaderSize + key_size) {
    return false;
  }

  if (value_offset + value_size + DeltaLogFooter::kSize > file_size) {
    return false;
  }
