...
class DeltaFileReader {
 public:
  ...
  DeltaFileReader(const DeltaFileReader&) = delete;
  DeltaFileReader& operator=(const DeltaFileReader&) = delete;
 ...
 private:
  DeltaFileReader(std::unique_ptr<RandomAccessFileReader>&& file_reader,
  ...
  static Status VerifyDelta(const Slice& record_slice, const Slice& user_key,
                            uint64_t value_size);

  static Status UncompressDeltaIfNeeded(const Slice& value_slice,
                                        CompressionType compression_type,
                                        MemoryAllocator* allocator,
                                        SystemClock* clock,
