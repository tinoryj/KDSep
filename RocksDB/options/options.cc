...
AdvancedColumnFamilyOptions::AdvancedColumnFamilyOptions(const Options& options)
    : max_write_buffer_number(options.max_write_buffer_number),
      ...
      enable_delta_garbage_collection(options.enable_delta_garbage_collection),
      ...
      delta_compaction_readahead_size(options.delta_compaction_readahead_size),
      delta_file_starting_level(options.delta_file_starting_level),
      delta_cache(options.delta_cache),
...
void ColumnFamilyOptions::Dump(Logger* log) const {
  ...
  if (blob_cache) {
    ...
    ROCKS_LOG_HEADER(log,
                     ...
                     prepopulate_blob_cache == PrepopulateBlobCache::kFlushOnly
                         ...
                         : "disabled");
    ROCKS_LOG_HEADER(log,
                     "                      Options.enable_delta_files: %s",
                     enable_delta_files ? "true" : "false");
    ROCKS_LOG_HEADER(
        log, "                          Options.min_delta_size: %" PRIu64,
        min_delta_size);
    ROCKS_LOG_HEADER(
        log, "                         Options.delta_file_size: %" PRIu64,
        delta_file_size);
    ROCKS_LOG_HEADER(log,
                     "                  Options.delta_compression_type: %s",
                     CompressionTypeToString(delta_compression_type).c_str());
    ROCKS_LOG_HEADER(log,
                     "         Options.enable_delta_garbage_collection: %s",
                     enable_delta_garbage_collection ? "true" : "false");
    ROCKS_LOG_HEADER(log,
                     "     Options.delta_garbage_collection_age_cutoff: %f",
                     delta_garbage_collection_age_cutoff);
    ROCKS_LOG_HEADER(log,
                     "Options.delta_garbage_collection_force_threshold: %f",
                     delta_garbage_collection_force_threshold);
    ROCKS_LOG_HEADER(
        log, "         Options.delta_compaction_readahead_size: %" PRIu64,
        delta_compaction_readahead_size);
    ROCKS_LOG_HEADER(log,
                     "               Options.delta_file_starting_level: %d",
                     delta_file_starting_level);
    if (delta_cache) {
      ROCKS_LOG_HEADER(log, "                          Options.delta_cache: %s",
                       delta_cache->Name());
      ROCKS_LOG_HEADER(log, "                          delta_cache options: %s",
                       delta_cache->GetPrintableOptions().c_str());
      ROCKS_LOG_HEADER(
          log, "                          delta_cache prepopulated: %s",
  ...
  ColumnFamilyOptions* ColumnFamilyOptions::OptimizeLevelStyleCompaction(
    ...
    // (memtable_memory_budget / 2). This will flush level 0 when it's bigger
    // than memtable_memory_budget.
    level0_file_num_compaction_trigger = 2;
    // doesn't really matter much, but we don't want to create too many files
    target_file_size_base = memtable_memory_budget / 8;
    // make Level1 size equal to Level0 size, so that L0->L1 compactions are
    // fast
