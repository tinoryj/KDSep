...
// users to use delta DB.

constexpr uint64_t kNoExpiration = std::numeric_limits<uint64_t>::max();

struct DeltaDBOptions {
  // Name of the directory under the base DB where deltas will be stored. Using
  // a directory where the base DB stores its SST files is not supported.
  // Default is "delta_dir"
  std::string delta_dir = "delta_dir";

  // whether the delta_dir path is relative or absolute.
  bool path_relative = true;

  // When max_db_size is reached, evict delta files to free up space
  // instead of returnning NoSpace error on write. Delta files will be
  // evicted from oldest to newest, based on file creation time.
  bool is_fifo = false;

  // Maximum size of the database (including SST files and delta files).
  //
  // Default: 0 (no limits)
  ...
  // then the delta buckets will be
  // first bucket is 1471542000 - 1471542600
  // second bucket is 1471542600 - 1471543200
  // and so on
  uint64_t ttl_range_secs = 3600;

  // The smallest value to store in delta log. Values smaller than this
  // threshold will be inlined in base DB together with the key.
  uint64_t min_delta_size = 0;

  // Allows OS to incrementally sync delta files to disk for every
  // bytes_per_sync bytes written. Users shouldn't rely on it for
  // persistency guarantee.
  uint64_t bytes_per_sync = 512 * 1024;

  // the target size of each delta file. File will become immutable
  // after it exceeds that size
  uint64_t delta_file_size = 256 * 1024 * 1024;

  // what compression to use for Delta's
  CompressionType compression = kNoCompression;

  // If enabled, DeltaDB cleans up stale deltas in non-TTL files during
  // compaction by rewriting the remaining live deltas to new files.
  bool enable_garbage_collection = false;

  // The cutoff in terms of delta file age for garbage collection. Deltas in
  // the oldest N non-TTL delta files will be rewritten when encountered during
...
class DeltaDB : public StackableDB {
 public:
  ...
  virtual void MultiGet(const ReadOptions& /*options*/,
    ...
    for (size_t i = 0; i < num_keys; ++i) {
      statuses[i] =
          Status::NotSupported("Delta DB doesn't support batched MultiGet");
    }
  }
  ...
  // Opening delta db.
  static Status Open(const Options& options, const DeltaDBOptions& bdb_options,
  ...
  virtual DeltaDBOptions GetDeltaDBOptions() const = 0;

  virtual Status SyncDeltaFiles() = 0;
