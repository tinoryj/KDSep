...
DeltaDumpTool::DeltaDumpTool()
    : reader_(nullptr), buffer_(nullptr), buffer_size_(0) {}

Status DeltaDumpTool::Run(const std::string& filename, DisplayType show_key,
                          DisplayType show_delta,
                          DisplayType show_uncompressed_delta,
                          bool show_summary) {
  ...
  s = DumpDeltaLogHeader(&offset, &compression);
  if (!s.ok()) {
    return s;
  }
  s = DumpDeltaLogFooter(file_size, &footer_offset);
  if (!s.ok()) {
    return s;
  }
  ...
  uint64_t total_uncompressed_delta_size = 0;
  if (show_key != DisplayType::kNone || show_summary) {
    while (offset < footer_offset) {
      s = DumpRecord(show_key, show_delta, show_uncompressed_delta,
                     show_summary, compression, &offset, &total_records,
                     &total_key_size, &total_delta_size,
                     &total_uncompressed_delta_size);
      if (!s.ok()) {
        break;
      }
    }
  ...
  if (show_summary) {
    ...
    fprintf(stdout, "  total delta size: %" PRIu64 "\n", total_delta_size);
    if (compression != kNoCompression) {
      fprintf(stdout, "  total raw delta size: %" PRIu64 "\n",
              total_uncompressed_delta_size);
    }
  }
  return s;
...
Status DeltaDumpTool::Read(uint64_t offset, size_t size, Slice* result) {
...
Status DeltaDumpTool::DumpDeltaLogHeader(uint64_t* offset,
                                         CompressionType* compression) {
  Slice slice;
  Status s = Read(0, DeltaLogHeader::kSize, &slice);
  if (!s.ok()) {
    return s;
  }
  ...
  fprintf(stdout, "Delta log header:\n");
  ...
  *offset = DeltaLogHeader::kSize;
  *compression = header.compression;
  return s;
}

Status DeltaDumpTool::DumpDeltaLogFooter(uint64_t file_size,
                                         uint64_t* footer_offset) {
  auto no_footer = [&]() {
    *footer_offset = file_size;
    fprintf(stdout, "No delta log footer.\n");
    return Status::OK();
  };
  ...
  Slice slice;
  *footer_offset = file_size - DeltaLogFooter::kSize;
  Status s = Read(*footer_offset, DeltaLogFooter::kSize, &slice);
  if (!s.ok()) {
    return s;
  }
  ...
  fprintf(stdout, "Delta log footer:\n");
  fprintf(stdout, "  Delta count       : %" PRIu64 "\n", footer.delta_count);
  fprintf(stdout, "  Expiration Range : %s\n",
          GetString(footer.expiration_range).c_str());
  return s;
}

Status DeltaDumpTool::DumpRecord(DisplayType show_key, DisplayType show_delta,
                                 DisplayType show_uncompressed_delta,
  ...
  Slice slice;
  Status s = Read(*offset, DeltaLogRecord::kHeaderSize, &slice);
  if (!s.ok()) {
    return s;
  }
  ...
  *offset += DeltaLogRecord::kHeaderSize;
  ...
  if (compression != kNoCompression &&
      (show_uncompressed_delta != DisplayType::kNone || show_summary)) {
  ...
  if (show_key != DisplayType::kNone) {
    ...
    if (show_delta != DisplayType::kNone) {
      fprintf(stdout, "  delta       : ");
      DumpSlice(Slice(slice.data() + static_cast<size_t>(key_size),
                      static_cast<size_t>(value_size)),
                show_delta);
    }
    if (show_uncompressed_delta != DisplayType::kNone) {
      fprintf(stdout, "  raw delta   : ");
      DumpSlice(Slice(uncompressed_value), show_uncompressed_delta);
    }
  }
  *offset += key_size + value_size;
  *total_records += 1;
  *total_key_size += key_size;
  *total_delta_size += value_size;
  *total_uncompressed_delta_size += uncompressed_value.size();
  return s;
}

void DeltaDumpTool::DumpSlice(const Slice s, DisplayType type) {
...
template <class T>
std::string DeltaDumpTool::GetString(std::pair<T, T> p) {
