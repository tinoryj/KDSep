...
class DeltaFile {
  friend class DeltaDBImpl;
  friend struct DeltaFileComparator;
  friend struct DeltaFileComparatorTTL;
  friend class DeltaIndexCompactionFilterBase;
  friend class DeltaIndexCompactionFilterGC;

 private:
  // access to parent
  const DeltaDBImpl* parent_{nullptr};
  ...
  // The file numbers of the SST files whose oldest delta file reference
  // points to this delta file.
  ...
  // Compression type of deltas in the file
  ...
  // TTL range of deltas in the file.
  ExpirationRange expiration_range_;

  // number of deltas in the file
  std::atomic<uint64_t> delta_count_{0};
  ...
  // no more deltas will be appended and the footer has been written out
  ...
  // Sequential/Append writer for deltas
  std::shared_ptr<DeltaLogWriter> log_writer_;
 ...
 public:
  ...
  DeltaFile(const DeltaDBImpl* parent, const std::string& bdir, uint64_t fnum,
            Logger* info_log);
  ...
  // Primary identifier for delta file.
  // once the file is created, this never changes
  uint64_t DeltaFileNumber() const { return file_number_; }

  // Get the set of SST files whose oldest delta file reference points to
  // this file.
  const std::unordered_set<uint64_t>& GetLinkedSstFiles() const {
    return linked_sst_files_;
  }

  // Link an SST file whose oldest delta file reference points to this file.
  ...
  // Unlink an SST file whose oldest delta file reference points to this file.
  ...
  uint64_t DeltaCount() const {
    return delta_count_.load(std::memory_order_acquire);
  }
  ...
  std::shared_ptr<DeltaLogWriter> GetWriter() const { return log_writer_; }

  // Read delta file header and footer. Return corruption if file header is
 ...
 private:
  ...
  Status SetFromFooterLocked(const DeltaLogFooter& footer);
  ...
  void SetDeltaCount(uint64_t bc) { delta_count_ = bc; }

  void DeltaRecordAdded(uint64_t record_size) {
    ++delta_count_;
