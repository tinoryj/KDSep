...
static const std::string kRocksDBDeltaFileExt = "delta";
...
std::string DeltaFileName(uint64_t number) {
  assert(number > 0);
  return MakeFileName(number, kRocksDBDeltaFileExt.c_str());
}

std::string DeltaFileName(const std::string& deltadirname, uint64_t number) {
  assert(number > 0);
  return MakeFileName(deltadirname, number, kRocksDBDeltaFileExt.c_str());
}

std::string DeltaFileName(const std::string& dbname,
                          const std::string& delta_dir, uint64_t number) {
  assert(number > 0);
  return MakeFileName(dbname + "/" + delta_dir, number,
                      kRocksDBDeltaFileExt.c_str());
}
  ...
  if (rest == "IDENTITY") {
    ...
    *type = is_temp_file ? kTempFile : kOptionsFile;
    ...
    if (suffix == Slice("log")) {
      ...
      *type = kBlobFile;
    } else if (suffix == Slice(kRocksDBDeltaFileExt)) {
      *type = kDeltaFile;
