...
for file in $*; do
    rd_rock=`grep "rocksdb.last.level.read.bytes\|rocksdb.non.last.level.read.bytes" $file | awk 'BEGIN {t=0;} {t+=$NF;} END {print $NF / 1024 / 1024 / 1024;}'`
    ...
    concatFunc "$rd_rock" "$r_blob" "$actual" "$act_bl" "$comp_w" "$flush" "$wal" "$rock_io" "$sst_num" "$rss" "$cache_gb" "$d_rw" "$br_sz" "$v_rw" "$tot_rw" "$thpt" "$file"
done
