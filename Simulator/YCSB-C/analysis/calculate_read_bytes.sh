...
echo "rd_rock"$'\t'"r_blob"$'\t'"actual"$'\t'"act_bl"$'\t'"comp_w"$'\t'"flush"$'\t'"wal"$'\t'"rock_io"$'\t'"sst_sz"$'\t'"rss_gb"$'\t'"cache-g"$'\t'"d_rw"$'\t'"br_sz"$'\t'"v_rw"$'\t'"tot_rw"$'\t'"thpt"$'\t'"fname"
...
#    echo $num1
#    echo $num2
    echo "$rd_rock"$'\t'"$r_blob"$'\t'"$actual"$'\t'"$act_bl"$'\t'"$comp_w"$'\t'"$flush"$'\t'"$wal"$'\t'"$rock_io"$'\t'"$sst_num"$'\t'"$rss"$'\t'"$cache_gb"$'\t'"$d_rw"$'\t'"$br_sz"$'\t'"$v_rw"$'\t'"$tot_rw"$'\t'"$thpt"$'\t'"$file"
done
