...
KDSepDB::KDSepDB(const char *dbfilename, const std::string &config_file_path) {
    ...
    if (directIO == true) {
        ...
        options_.fileOperationMethod_ = (directReads) ?
            KDSEP_NAMESPACE::kDirectIO : KDSEP_NAMESPACE::kAlignLinuxIO;
    } else {
    ...
    options_.deltaKV_merge_operation_ptr.reset(new KDSEP_NAMESPACE::DeltaKVFieldUpdateMergeOperator);
...
int KDSepDB::Read(const std::string &table, const std::string &key,
    ...
    KDSEP_NAMESPACE::StatsRecorder::getInstance()->timeProcess(KDSEP_NAMESPACE::StatsType::DELTAKV_GET, tv);
    gettimeofday(&tv_, 0);
...
int KDSepDB::Scan(const std::string &table, const std::string &key, int len,
    ...
    KDSEP_NAMESPACE::StatsRecorder::getInstance()->timeProcess(KDSEP_NAMESPACE::StatsType::DELTAKV_SCAN, tv);
    gettimeofday(&tv_, 0);
    return 1;
}
...
//    }
    ...
    KDSEP_NAMESPACE::StatsRecorder::getInstance()->timeProcess(KDSEP_NAMESPACE::StatsType::DELTAKV_PUT, tv);
    gettimeofday(&tv_, 0);
    if (!status) {
        cerr << "insert error"
             << endl;
...
int KDSepDB::Update(const std::string &table, const std::string &key,
    ...
    KDSEP_NAMESPACE::StatsRecorder::getInstance()->timeProcess(KDSEP_NAMESPACE::StatsType::DELTAKV_MERGE, tv);
    gettimeofday(&tv_, 0);
