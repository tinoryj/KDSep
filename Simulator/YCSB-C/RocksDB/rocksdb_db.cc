...
RocksDB::RocksDB(const char *dbfilename, const std::string &config_file_path) {
  ...
  bool keyValueSeparation = config.getDirectIO();
  ...
  if (keyValueSeparation == true) {
    ...
    // options_.min_blob_size = ;                     // Default 0
    options_.blob_file_size = 64 * 1024 * 1024;       // Default 256*1024*1024
    ...
    // options_.blob_compaction_readahead_size = ;           // Default 0
    // options_.blob_file_starting_level = ;                 // Default 0
    // options_.blob_cache = ;                               // Default nullptr
    // options_.prepopulate_blob_cache = ;                   // Default kDisable
  }
  ...
  options_.target_file_size_base = 8 << 20;
