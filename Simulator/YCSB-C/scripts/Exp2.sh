...
for runMode in "${runModeSet[@]}"; do
    ...
    for index in "${indexSet[@]}"; do
        ...
        scripts/runTestNew.sh $runMode req40M op5M fc10 fl100 cache1024 threads$threadNumber workerT$works gcT$gcs batchSize2K round1 readRatio0.$index bucketNum$bucketNumber Exp$ExpName
    done
done
