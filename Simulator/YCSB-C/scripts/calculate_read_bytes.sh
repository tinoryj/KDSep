...
echo "r_lsm"$'\t'"r_blob"$'\t'"actual"$'\t'"act_bl"$'\t'"comp_w"$'\t'"rock_io"$'\t'"sst_sz"$'\t'"rss_gb"$'\t'"cache-g"$'\t'"d_read"$'\t'"d_size"$'\t'"br_sz"$'\t'"v_read"$'\t'"thpt"$'\t'"fname"
...
for file in $*; do
    ...
    rock_io=`echo $actual $comp_w | awk '{if ($1==0) print -1; else print $1+$2;}' | cut -c1-7`
...
#    echo $num1
#    echo $num2
    echo "`echo $num1 $num2 | awk '{print ($1+$2) / 1024.0 / 1024.0 / 1024.0;}' | cut -c1-7`"$'\t'"$r_blob"$'\t'"$actual"$'\t'"$actual_blob"$'\t'"$comp_w"$'\t'"$rock_io"$'\t'"$sst_num"$'\t'"$resident"$'\t'"$cache_gb"$'\t'"$deltaRead"$'\t'"$deltaSize"$'\t'"$br_sz"$'\t'"$v_read"$'\t'"$thpt"$'\t'"$file"
done
