...
class WriteBatchHandlerJni
 ...
 public:
  ...
  /**
   * Get the Java Method: WriteBatch.Handler#putDeltaLogIndex
   *
   * @param env A pointer to the Java environment
   *
   * @return The Java Method ID or nullptr if the class or method id could not
   *     be retrieved
   */
  static jmethodID getPutDeltaLogIndexCfMethodId(JNIEnv* env) {
    jclass jclazz = getJClass(env);
    if (jclazz == nullptr) {
      // exception occurred accessing class
      return nullptr;
    ...
    static jmethodID mid =
        env->GetMethodID(jclazz, "putDeltaLogIndex", "(I[B[B)V");
    assert(mid != nullptr);
    return mid;
  }
