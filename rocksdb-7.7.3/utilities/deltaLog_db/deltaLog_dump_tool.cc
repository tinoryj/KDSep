...
namespace deltaLog_db {

DeltaLogDumpTool::DeltaLogDumpTool()
    : reader_(nullptr), buffer_(nullptr), buffer_size_(0) {}

Status DeltaLogDumpTool::Run(const std::string& filename, DisplayType show_key,
                             DisplayType show_deltaLog,
                             DisplayType show_uncompressed_deltaLog,
                             bool show_summary) {
  ...
  s = DumpDeltaLogLogHeader(&offset, &compression);
  if (!s.ok()) {
    return s;
  }
  s = DumpDeltaLogLogFooter(file_size, &footer_offset);
  if (!s.ok()) {
    return s;
  }
  uint64_t total_records = 0;
  uint64_t total_key_size = 0;
  uint64_t total_deltaLog_size = 0;
  uint64_t total_uncompressed_deltaLog_size = 0;
  if (show_key != DisplayType::kNone || show_summary) {
    while (offset < footer_offset) {
      s = DumpRecord(show_key, show_deltaLog, show_uncompressed_deltaLog,
                     show_summary, compression, &offset, &total_records,
                     &total_key_size, &total_deltaLog_size,
                     &total_uncompressed_deltaLog_size);
      if (!s.ok()) {
        break;
      }
    }
  ...
  if (show_summary) {
    ...
    fprintf(stdout, "  total deltaLog size: %" PRIu64 "\n",
            total_deltaLog_size);
    if (compression != kNoCompression) {
      fprintf(stdout, "  total raw deltaLog size: %" PRIu64 "\n",
              total_uncompressed_deltaLog_size);
    }
  }
  return s;
...
Status DeltaLogDumpTool::Read(uint64_t offset, size_t size, Slice* result) {
...
Status DeltaLogDumpTool::DumpDeltaLogLogHeader(uint64_t* offset,
  ...
  fprintf(stdout, "DeltaLog log header:\n");
...
Status DeltaLogDumpTool::DumpDeltaLogLogFooter(uint64_t file_size,
  ...
  *footer_offset = file_size - DeltaLogFooter::kSize_;
  ...
  fprintf(stdout, "DeltaLog log footer:\n");
  fprintf(stdout, "  DeltaLog count       : %" PRIu64 "\n",
          footer.deltaLog_count);
  fprintf(stdout, "  Expiration Range : %s\n",
          GetString(footer.expiration_range).c_str());
  return s;
}

Status DeltaLogDumpTool::DumpRecord(
    DisplayType show_key, DisplayType show_deltaLog,
    DisplayType show_uncompressed_deltaLog, bool show_summary,
    CompressionType compression, uint64_t* offset, uint64_t* total_records,
    uint64_t* total_key_size, uint64_t* total_deltaLog_size,
    uint64_t* total_uncompressed_deltaLog_size) {
  ...
  *offset += DeltaLogRecord::kHeaderSize_;
  ...
  if (compression != kNoCompression &&
      (show_uncompressed_deltaLog != DisplayType::kNone || show_summary)) {
  ...
  if (show_key != DisplayType::kNone) {
    ...
    if (show_deltaLog != DisplayType::kNone) {
      fprintf(stdout, "  deltaLog       : ");
      DumpSlice(Slice(slice.data() + static_cast<size_t>(key_size),
                      static_cast<size_t>(value_size)),
                show_deltaLog);
    }
    if (show_uncompressed_deltaLog != DisplayType::kNone) {
      fprintf(stdout, "  raw deltaLog   : ");
      DumpSlice(Slice(uncompressed_value), show_uncompressed_deltaLog);
    }
  }
  *offset += key_size + value_size;
  *total_records += 1;
  *total_key_size += key_size;
  *total_deltaLog_size += value_size;
  *total_uncompressed_deltaLog_size += uncompressed_value.size();
  return s;
}

void DeltaLogDumpTool::DumpSlice(const Slice s, DisplayType type) {
...
template <class T>
std::string DeltaLogDumpTool::GetString(std::pair<T, T> p) {
