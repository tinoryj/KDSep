...
// kTypeDeltaLogIndex is a value type used by DeltaLogDB only. The base rocksdb
...
// doesn't understand format of actual deltaLog index (the value).
class DBDeltaLogIndexTest : public DBTestBase {
 public:
  enum Tier {
    kMemtable = 0,
  ...
  DBDeltaLogIndexTest()
      : DBTestBase("db_deltaLog_index_test", /*env_do_fsync=*/true) {}
  ...
  Status PutDeltaLogIndex(WriteBatch* batch, const Slice& key,
                          const Slice& deltaLog_index) {
    return WriteBatchInternal::PutDeltaLogIndex(batch, cfd()->GetID(), key,
                                                deltaLog_index);
  }
  ...
  std::string GetImpl(const Slice& key, bool* is_deltaLog_index = nullptr,
    ...
    get_impl_options.is_deltaLog_index = is_deltaLog_index;
  ...
  std::string GetDeltaLogIndex(const Slice& key,
                               const Snapshot* snapshot = nullptr) {
    bool is_deltaLog_index = false;
    std::string value = GetImpl(key, &is_deltaLog_index, snapshot);
  ...
  ArenaWrappedDBIter* GetDeltaLogIterator() {
    return dbfull()->NewIteratorImpl(
        ReadOptions(), cfd(), dbfull()->GetLatestSequenceNumber(),
        nullptr /*read_callback*/, true /*expose_deltaLog_index*/);
  }
...
// Note: the following test case pertains to the StackableDB-based DeltaLogDB
// implementation. We should be able to write kTypeDeltaLogIndex to memtables
// and SST files.
TEST_F(DBDeltaLogIndexTest, Write) {
  for (auto tier : kAllTiers) {
    ...
    for (size_t i = 1; i <= num_key_values; ++i) {
      ...
      DeltaLogIndex::EncodeInlinedTTL(&deltaLog_index,
                                      /* expiration */ 9876543210,
                                      "deltaLog" + std::to_string(i));

      key_values.emplace_back(std::move(key), std::move(deltaLog_index));
    }

    for (const auto& key_value : key_values) {
      WriteBatch batch;
      ASSERT_OK(PutDeltaLogIndex(&batch, key_value.first, key_value.second));
    ...
    for (const auto& key_value : key_values) {
      ASSERT_EQ(GetDeltaLogIndex(key_value.first), key_value.second);
    }
  }
}

// Note: the following test case pertains to the StackableDB-based DeltaLogDB
// implementation. Get should be able to return deltaLog index if
// is_deltaLog_index is provided, otherwise it should return
// Status::NotSupported (when reading from memtable) or Status::Corruption (when
// reading from SST). Reading from SST returns Corruption because we can't
// differentiate between the application accidentally opening the base DB of a
// stacked DeltaLogDB and actual corruption when using the integrated
// DeltaLogDB.
TEST_F(DBDeltaLogIndexTest, Get) {
  std::string deltaLog_index;
  DeltaLogIndex::EncodeInlinedTTL(&deltaLog_index, /* expiration */ 9876543210,
                                  "deltaLog");
  ...
  for (auto tier : kAllTiers) {
    ...
    ASSERT_OK(PutDeltaLogIndex(&batch, "deltaLog_key", deltaLog_index));
    ASSERT_OK(Write(&batch));
    ...
    ASSERT_EQ("value", GetImpl("key", &is_deltaLog_index));
    ASSERT_FALSE(is_deltaLog_index);

    // Verify deltaLog index
    if (tier <= kImmutableMemtables) {
      ASSERT_TRUE(Get("deltaLog_key", &value).IsNotSupported());
      ASSERT_EQ("NOT_SUPPORTED", GetImpl("deltaLog_key"));
    } else {
      ASSERT_TRUE(Get("deltaLog_key", &value).IsCorruption());
      ASSERT_EQ("CORRUPTION", GetImpl("deltaLog_key"));
    }
    ASSERT_EQ(deltaLog_index, GetImpl("deltaLog_key", &is_deltaLog_index));
    ASSERT_TRUE(is_deltaLog_index);
  }
}

// Note: the following test case pertains to the StackableDB-based DeltaLogDB
// implementation. Get should NOT return Status::NotSupported/Status::Corruption
// if deltaLog index is updated with a normal value. See the test case above for
// more details.
TEST_F(DBDeltaLogIndexTest, Updated) {
  std::string deltaLog_index;
  DeltaLogIndex::EncodeInlinedTTL(&deltaLog_index, /* expiration */ 9876543210,
                                  "deltaLog");
  ...
  for (auto tier : kAllTiers) {
    ...
    for (int i = 0; i < 10; i++) {
      ASSERT_OK(
          PutDeltaLogIndex(&batch, "key" + std::to_string(i), deltaLog_index));
    }
    ASSERT_OK(Write(&batch));
    // Avoid deltaLog values from being purged.
    ...
    MoveDataTo(tier);
    for (int i = 0; i < 10; i++) {
      ASSERT_EQ(deltaLog_index,
                GetDeltaLogIndex("key" + std::to_string(i), snapshot));
    }
    ...
    ASSERT_EQ(deltaLog_index, GetDeltaLogIndex("key9"));
    dbfull()->ReleaseSnapshot(snapshot);
  }
}

// Note: the following test case pertains to the StackableDB-based DeltaLogDB
// implementation. When a deltaLog iterator is used, it should set the
// expose_deltaLog_index flag for the underlying DBIter, and retrieve/return the
// corresponding deltaLog value. If a regular DBIter is created (i.e.
// expose_deltaLog_index is not set), it should return Status::Corruption.
TEST_F(DBDeltaLogIndexTest, Iterate) {
  const std::vector<std::vector<ValueType>> data = {
      /*00*/ {kTypeValue},
      /*01*/ {kTypeDeltaLogIndex},
      /*02*/ {kTypeValue},
      /*03*/ {kTypeDeltaLogIndex, kTypeValue},
      /*04*/ {kTypeValue},
      /*05*/ {kTypeValue, kTypeDeltaLogIndex},
      /*06*/ {kTypeValue},
      /*07*/ {kTypeDeletion, kTypeDeltaLogIndex},
      /*08*/ {kTypeValue},
      /*09*/ {kTypeSingleDeletion, kTypeDeltaLogIndex},
      /*10*/ {kTypeValue},
      /*11*/ {kTypeMerge, kTypeMerge, kTypeMerge, kTypeDeltaLogIndex},
      /*12*/ {kTypeValue},
      /*13*/
      {kTypeMerge, kTypeMerge, kTypeMerge, kTypeDeletion, kTypeDeltaLogIndex},
      /*14*/ {kTypeValue},
      /*15*/ {kTypeDeltaLogIndex},
      /*16*/ {kTypeValue},
  };
  ...
  auto create_deltaLog_iterator = [&]() -> Iterator* {
    return GetDeltaLogIterator();
  };

  auto check_is_deltaLog = [&](bool is_deltaLog) {
    return [is_deltaLog](Iterator* iterator) {
      ASSERT_EQ(is_deltaLog,
                reinterpret_cast<ArenaWrappedDBIter*>(iterator)->IsDeltaLog());
    };
  };
  ...
  for (auto tier : {Tier::kMemtable} /*kAllTiers*/) {
      ...
      for (int j = static_cast<int>(data[i].size()) - 1; j >= 0; j--) {
        ...
        switch (data[i][j]) {
          ...
          case kTypeDeltaLogIndex:
            ASSERT_OK(PutDeltaLogIndex(&batch, key, value));
            ASSERT_OK(Write(&batch));
            break;
          default:
            FAIL();
    ...
    // Iterator with deltaLog support
    verify(1, Status::kOk, get_value(1, 0), get_value(1, 0),
           create_deltaLog_iterator, check_is_deltaLog(true));
    verify(3, Status::kOk, get_value(3, 0), get_value(3, 0),
           create_deltaLog_iterator, check_is_deltaLog(true));
    verify(5, Status::kOk, get_value(5, 0), get_value(5, 0),
           create_deltaLog_iterator, check_is_deltaLog(false));
    verify(7, Status::kOk, get_value(8, 0), get_value(6, 0),
           create_deltaLog_iterator, check_is_deltaLog(false));
    verify(9, Status::kOk, get_value(10, 0), get_value(8, 0),
           create_deltaLog_iterator, check_is_deltaLog(false));
    if (tier <= kImmutableMemtables) {
      verify(11, Status::kNotSupported, "", "", create_deltaLog_iterator);
    } else {
      verify(11, Status::kCorruption, "", "", create_deltaLog_iterator);
    }
    verify(13, Status::kOk,
           get_value(13, 2) + "," + get_value(13, 1) + "," + get_value(13, 0),
           get_value(13, 2) + "," + get_value(13, 1) + "," + get_value(13, 0),
...
#ifndef ROCKSDB_LITE
    // Iterator with deltaLog support and using seek.
...
TEST_F(DBDeltaLogIndexTest, IntegratedDeltaLogIterate) {
