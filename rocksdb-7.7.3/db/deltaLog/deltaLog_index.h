...
// DeltaLogIndex is a pointer to the deltaLog and metadata of the deltaLog. The
// index is stored in base DB as ValueType::kTypeDeltaLogIndex. There are three
// types of deltaLog index:
//
//    kInlinedTTL:
//      +------+------------+---------------+
...
class DeltaLogIndex {
 public:
  ...
  DeltaLogIndex() : type_(Type::kUnknown) {}
  ...
  bool HasTTL() const {
    return type_ == Type::kInlinedTTL || type_ == Type::kDeltaLogTTL;
  }
  ...
  Status DecodeFrom(Slice slice) {
    ...
    if (IsInlined()) {
      ...
      if (GetVarint64(&slice, &file_number_) && GetVarint64(&slice, &offset_) &&
        ...
        return Status::Corruption(kErrorMessage, "Corrupted deltaLog offset");
      }
    }
    return Status::OK();
  }
  ...
  std::string DebugString(bool output_hex) const {
    ...
    if (IsInlined()) {
      oss << "[inlined deltaLog] value:" << value_.ToString(output_hex);
    } else {
      oss << "[deltaLog ref] file:" << file_number_ << " offset:" << offset_
          << " size:" << size_
  ...
  static void EncodeDeltaLog(std::string* dst, uint64_t file_number,
                             uint64_t offset, uint64_t size,
                             CompressionType compression) {
    assert(dst != nullptr);
    dst->clear();
    dst->reserve(kMaxVarint64Length * 3 + 2);
    dst->push_back(static_cast<char>(Type::kDeltaLog));
  ...
  static void EncodeDeltaLogTTL(std::string* dst, uint64_t expiration,
                                uint64_t file_number, uint64_t offset,
                                uint64_t size, CompressionType compression) {
    assert(dst != nullptr);
    dst->clear();
    dst->reserve(kMaxVarint64Length * 4 + 2);
    dst->push_back(static_cast<char>(Type::kDeltaLogTTL));
    PutVarint64(dst, expiration);
    PutVarint64(dst, file_number);
    PutVarint64(dst, offset);
    PutVarint64(dst, size);
