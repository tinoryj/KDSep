...
// generated by a compaction. It parses the keys and deltaLog references in the
// input and output of a compaction, and aggregates the "inflow" and "outflow"
// on a per-deltaLog file basis. The amount of additional garbage for any given
// deltaLog file can then be computed by subtracting the outflow from the
// inflow.
class DeltaLogGarbageMeter {
 public:
  // A class to store the number and total size of deltaLogs on a per-deltaLog
  // file basis.
  class DeltaLogStats {
   public:
  ...
  // amount of additional garbage for a given deltaLog file.
  class DeltaLogInOutFlow {
   public:
    ...
    const DeltaLogStats& GetInFlow() const { return in_flow_; }
    const DeltaLogStats& GetOutFlow() const { return out_flow_; }
