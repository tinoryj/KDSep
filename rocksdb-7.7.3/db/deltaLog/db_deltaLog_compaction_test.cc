...
class DBDeltaLogCompactionTest : public DBTestBase {
 public:
  explicit DBDeltaLogCompactionTest()
      : DBTestBase("db_deltaLog_compaction_test", /*env_do_fsync=*/false) {}
...
class FilterByKeyLength : public CompactionFilter {
 public:
  ...
  CompactionFilter::Decision FilterDeltaLogByKey(
...
class BadDeltaLogCompactionFilter : public CompactionFilter {
 public:
  explicit BadDeltaLogCompactionFilter(std::string prefix,
                                       CompactionFilter::Decision filter_by_key,
                                       CompactionFilter::Decision filter_v2)
      : prefix_(std::move(prefix)),
        filter_deltaLog_by_key_(filter_by_key),
  ...
  CompactionFilter::Decision FilterDeltaLogByKey(
    ...
    return filter_deltaLog_by_key_;
  }
  CompactionFilter::Decision FilterV2(
      int /*level*/, const Slice& /*key*/, ValueType /*value_type*/,
      const Slice& /*existing_value*/, std::string* /*new_value*/,
 ...
 private:
  ...
  const CompactionFilter::Decision filter_deltaLog_by_key_;
...
CompactionFilter::Decision ValueBlindWriteFilter::FilterDeltaLogByKey(
    int /*level*/, const Slice& /*key*/, std::string* new_value,
...
CompactionFilter::Decision ValueMutationFilter::FilterV2(
    ...
    const Slice& existing_value, std::string* new_value,
    std::string* /*skip_until*/) const {
  assert(CompactionFilter::ValueType::kDeltaLogIndex != value_type);
...
class DBDeltaLogBadCompactionFilterTest
    : public DBDeltaLogCompactionTest,
      public testing::WithParamInterface<
          std::tuple<std::string, CompactionFilter::Decision,
                     CompactionFilter::Decision>> {
 public:
  explicit DBDeltaLogBadCompactionFilterTest()
      : compaction_filter_guard_(new BadDeltaLogCompactionFilter(
...
INSTANTIATE_TEST_CASE_P(
    BadCompactionFilter, DBDeltaLogBadCompactionFilterTest,
    testing::Combine(
        testing::Values("a"),
        testing::Values(CompactionFilter::Decision::kChangeDeltaLogIndex,
                        CompactionFilter::Decision::kIOError),
        testing::Values(CompactionFilter::Decision::kUndetermined,
                        CompactionFilter::Decision::kChangeDeltaLogIndex,
                        CompactionFilter::Decision::kIOError)));

TEST_F(DBDeltaLogCompactionTest, FilterByKeyLength) {
  ...
  constexpr char deltaLog_value[] = "value";

  DestroyAndReopen(options);
  ASSERT_OK(Put(short_key, deltaLog_value));
  ASSERT_OK(Put(long_key, deltaLog_value));
  ASSERT_OK(Flush());
  ...
  // this involves neither reading nor writing deltaLogs
  ASSERT_EQ(compaction_stats[1].bytes_read_deltaLog, 0);
  ASSERT_EQ(compaction_stats[1].bytes_written_deltaLog, 0);
#endif  // ROCKSDB_LITE

  Close();
}

TEST_F(DBDeltaLogCompactionTest, FilterByValueLength) {
  Options options = GetDefaultOptions();
  options.enable_deltaLog_files = true;
  options.min_deltaLog_size = 5;
  options.create_if_missing = true;
  ...
  // this involves reading but not writing deltaLogs
  ASSERT_GT(compaction_stats[1].bytes_read_deltaLog, 0);
  ASSERT_EQ(compaction_stats[1].bytes_written_deltaLog, 0);
#endif  // ROCKSDB_LITE
...
#ifndef ROCKSDB_LITE
TEST_F(DBDeltaLogCompactionTest, DeltaLogCompactWithStartingLevel) {
  Options options = GetDefaultOptions();

  options.enable_deltaLog_files = true;
  options.min_deltaLog_size = 1000;
  options.deltaLog_file_starting_level = 5;
  options.create_if_missing = true;
  ...
  ASSERT_EQ(0, GetDeltaLogFileNumbers().size());
  ...
  // No deltaLog file should be created since deltaLog_file_starting_level is 5.
  ASSERT_EQ(0, GetDeltaLogFileNumbers().size());
  ASSERT_EQ(0, NumTableFilesAtLevel(/*level=*/0));
  ASSERT_EQ(4, NumTableFilesAtLevel(/*level=*/1));

  {
    options.deltaLog_file_starting_level = 1;
    DestroyAndReopen(options);
    ...
    ASSERT_OK(db_->CompactRange(CompactRangeOptions(), /*begin=*/nullptr,
                                /*end=*/nullptr));
    // The compaction's output level equals to deltaLog_file_starting_level.
    ASSERT_EQ(1, GetDeltaLogFileNumbers().size());
    ASSERT_EQ(0, NumTableFilesAtLevel(/*level=*/0));
    ASSERT_EQ(4, NumTableFilesAtLevel(/*level=*/1));
  }
...
TEST_F(DBDeltaLogCompactionTest, BlindWriteFilter) {
  Options options = GetDefaultOptions();
  options.enable_deltaLog_files = true;
  options.min_deltaLog_size = 0;
  options.create_if_missing = true;
  constexpr char new_deltaLog_value[] = "new_deltaLog_value";
  std::unique_ptr<CompactionFilter> compaction_filter_guard(
      new ValueBlindWriteFilter(new_deltaLog_value));
  options.compaction_filter = compaction_filter_guard.get();
  DestroyAndReopen(options);
  ...
  for (const auto& key : keys) {
    ASSERT_EQ(new_deltaLog_value, Get(key));
  }
  ...
  // Filter unconditionally changes value in FilterDeltaLogByKey;
  // this involves writing but not reading deltaLogs
  ASSERT_EQ(compaction_stats[1].bytes_read_deltaLog, 0);
  ASSERT_GT(compaction_stats[1].bytes_written_deltaLog, 0);
#endif  // ROCKSDB_LITE

  Close();
}

TEST_F(DBDeltaLogCompactionTest, SkipUntilFilter) {
  Options options = GetDefaultOptions();
  options.enable_deltaLog_files = true;
  ...
  SyncPoint::GetInstance()->SetCallBack(
      "DeltaLogCountingIterator::UpdateAndCountDeltaLogIfNeeded:ProcessInFlow",
...
TEST_P(DBDeltaLogBadCompactionFilterTest, BadDecisionFromCompactionFilter) {
  Options options = GetDefaultOptions();
  options.enable_deltaLog_files = true;
  options.min_deltaLog_size = 0;
  options.create_if_missing = true;
...
TEST_F(DBDeltaLogCompactionTest, CompactionFilter_InlinedTTLIndex) {
  Options options = GetDefaultOptions();
  options.create_if_missing = true;
  options.enable_deltaLog_files = true;
  options.min_deltaLog_size = 0;
...
TEST_F(DBDeltaLogCompactionTest, CompactionFilter) {
  Options options = GetDefaultOptions();
  options.create_if_missing = true;
  options.enable_deltaLog_files = true;
  options.min_deltaLog_size = 0;
  ...
  // this involves reading and writing deltaLogs
  ASSERT_GT(compaction_stats[1].bytes_read_deltaLog, 0);
  ASSERT_GT(compaction_stats[1].bytes_written_deltaLog, 0);
#endif  // ROCKSDB_LITE
...
TEST_F(DBDeltaLogCompactionTest, CorruptedDeltaLogIndex) {
  Options options = GetDefaultOptions();
  options.create_if_missing = true;
  options.enable_deltaLog_files = true;
  options.min_deltaLog_size = 0;
  ...
  SyncPoint::GetInstance()->SetCallBack(
      "CompactionIterator::InvokeFilterIfNeeded::TamperWithDeltaLogIndex",
      [](void* arg) {
...
TEST_F(DBDeltaLogCompactionTest, CompactionFilterReadDeltaLogAndKeep) {
  Options options = GetDefaultOptions();
  options.create_if_missing = true;
  options.enable_deltaLog_files = true;
  options.min_deltaLog_size = 0;
  ...
  ASSERT_OK(Flush());
  std::vector<uint64_t> deltaLog_files = GetDeltaLogFileNumbers();
  ASSERT_EQ(1, deltaLog_files.size());
  ASSERT_OK(db_->CompactRange(CompactRangeOptions(), /*begin=*/nullptr,
                              /*end=*/nullptr));
  ASSERT_EQ(deltaLog_files, GetDeltaLogFileNumbers());
...
TEST_F(DBDeltaLogCompactionTest, TrackGarbage) {
  Options options = GetDefaultOptions();
  options.enable_deltaLog_files = true;

  Reopen(options);

  // First table+deltaLog file pair: 4 deltaLogs with different keys
  ...
  // Second table+deltaLog file pair: overwrite 2 existing keys
  ...
  // Compact them together. The first deltaLog file should have 2 garbage
  // deltaLogs corresponding to the 2 overwritten keys.
  ...
  ASSERT_EQ(deltaLog_files.size(), 2);

  {
    const auto& meta = deltaLog_files.front();
    assert(meta);
    ...
    ASSERT_EQ(meta->GetTotalDeltaLogCount(), 4);
    ASSERT_EQ(meta->GetTotalDeltaLogBytes(),
              first_expected_bytes + second_expected_bytes +
                  third_expected_bytes + fourth_expected_bytes);
    ASSERT_EQ(meta->GetGarbageDeltaLogCount(), 2);
    ASSERT_EQ(meta->GetGarbageDeltaLogBytes(),
              first_expected_bytes + second_expected_bytes);
  }

  {
    const auto& meta = deltaLog_files.back();
    assert(meta);
    ...
    ASSERT_EQ(meta->GetTotalDeltaLogCount(), 2);
    ASSERT_EQ(meta->GetTotalDeltaLogBytes(),
              new_first_expected_bytes + new_second_expected_bytes);
    ASSERT_EQ(meta->GetGarbageDeltaLogCount(), 0);
    ASSERT_EQ(meta->GetGarbageDeltaLogBytes(), 0);
  }
}

TEST_F(DBDeltaLogCompactionTest, MergeDeltaLogWithBase) {
  Options options = GetDefaultOptions();
  options.enable_deltaLog_files = true;
  options.min_deltaLog_size = 0;
...
TEST_F(DBDeltaLogCompactionTest, CompactionReadaheadGarbageCollection) {
  Options options = GetDefaultOptions();
  options.enable_deltaLog_files = true;
  options.min_deltaLog_size = 0;
  options.enable_deltaLog_garbage_collection = true;
  options.deltaLog_garbage_collection_age_cutoff = 1.0;
  options.deltaLog_compaction_readahead_size = 1 << 10;
  options.disable_auto_compactions = true;
...
TEST_F(DBDeltaLogCompactionTest, CompactionReadaheadFilter) {
  Options options = GetDefaultOptions();
...
TEST_F(DBDeltaLogCompactionTest, CompactionReadaheadMerge) {
  Options options = GetDefaultOptions();
  options.enable_deltaLog_files = true;
  options.min_deltaLog_size = 0;
  options.deltaLog_compaction_readahead_size = 1 << 10;
...
TEST_F(DBDeltaLogCompactionTest, CompactionDoNotFillCache) {
  Options options = GetDefaultOptions();
  ...
  ASSERT_EQ(options.statistics->getTickerCount(DELTALOG_DB_CACHE_ADD), 0);
